    ...
    <div className="cosmic-theme min-h-[100dvh] w-full max-w-[390px] mx-auto overflow-hidden flex flex-col font-sans text-foreground bg-aurora relative shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-24 scrollbar-hide z-10">
        ...
        <div className="mt-4 mb-8">
          <h2 className="font-display text-sm font-semibold text-white/70 uppercase tracking-wider mb-4">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-end mb-4">
            <h2 className="font-display text-sm font-semibold text-white/70 uppercase tracking-wider">
              Your Guides
            </h2>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide -mx-6 px-6">
            {[
              ...
              <div
                key={i}
                className={`flex-shrink-0 w-28 rounded-2xl glass-panel ${tutor.bg} ${tutor.border} p-4 flex flex-col items-center justify-center text-center animate-float`}
                style={{ animationDelay: `${i * 0.5}s` }}
              >
                <div
                  className={`w-12 h-12 rounded-full bg-background/50 flex items-center justify-center mb-3 shadow-[0_0_10px_rgba(255,255,255,0.05)]`}
                >
        ...
        <div>
          ...
          <div className="space-y-3">
            {[
              {
                title: "Quiet Focus",
                desc: "Completed a 5-min calming exercise",
                icon: <Award className="w-4 h-4 text-secondary" />,
              },
              {
                title: "Word Finder",
                desc: "Found 10 new vocabulary words",
                icon: <Zap className="w-4 h-4 text-primary" />,
              },
            ].map((item, i) => (
              <div
                key={i}
                className="glass-panel border-white/5 rounded-xl p-4 flex items-center gap-4"
              >
...
function Badge({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={`inline-flex items-center rounded-md font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ${className}`}
    >
      {children}
    </span>
  );
