...
> **Sprint 8 remaining (not yet migrated):** engagement/badges/sensory (REST via engagement-svc), homework-help/calm sessions, the standards/skill-graph surface, notification prefs/digests, and the standalone aux stores (`audit-store`, `household-store`, `idp-store`, `learner-pin-store`, `messages-store`, `mfa-store`, `parent-phone-store`, `user-roles-store`, `staff-invites`, `team-invites`). Identity/MFA/IDP belong to identity-svc (REST). These remain `db()`-direct and are tracked for a follow-up.
