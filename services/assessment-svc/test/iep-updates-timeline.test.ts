...
test(
  "timeline: cross-tenant teacher in Tenant B is forbidden from Tenant A learner",
  { skip: SKIP },
  async () => {
...
test(
  "timeline: same-tenant teacher who isn't on the IEP team is forbidden",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "timeline: parent of the learner can read their child's timeline",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "amendment proposal snapshots the CURRENT profile + goals (not the proposed delta)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "timeline: bogus learner UUID returns 404, not 403, so attackers can't tell tenants apart",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "timeline: malformed learner id returns 400 before any DB lookup",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
async function seedNotesAndReports(db: any, f: Fixture) {
  ...
  await db.insert(iepProgressNotes).values([
    {
      iepProfileId: f.profileA,
      learnerId: f.learnerA,
      authorId: f.teacherA,
      body: "PARENT_NOTE",
      visibility: "parent",
    },
  ...
  await db.insert(iepProgressReports).values([
    {
      ...
      learnerId: f.learnerA,
      period: "Q1",
      narrative: "DRAFT_REPORT",
      status: "draft",
      createdBy: f.caseManagerA,
    },
    {
      iepProfileId: f.profileA,
      learnerId: f.learnerA,
      period: "Q2",
      narrative: "SENT_REPORT",
      status: "sent",
      createdBy: f.caseManagerA,
      sentBy: f.caseManagerA,
      sentAt: new Date(),
    },
  ] as any);
}
...
test(
  "timeline: parent sees only parent-visible notes and only sent reports",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      const reportNarratives = items
        .filter((i) => i.type === "report")
        .map((i) => i.payload.narrative);
      assert.deepEqual(
        reportNarratives.sort(),
        ["SENT_REPORT"],
...
test(
  "timeline: IEP team member sees the full feed including internal notes and draft reports",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "timeline: district admin (no team membership) sees parent+team notes but never internal, and only sent reports",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "timeline: PLATFORM_ADMIN sees the full feed even without team membership",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
