...
async function cleanup(db: any, f: Fixture) {
  ...
  const { tenants, users, learners, learnerTeachers, iepEvaluations } = await import("@aivo/db");
  await db
    .delete(iepEvaluations)
    .where(inArray(iepEvaluations.learnerId, [f.learnerAId, f.learnerBId]));
...
test(
  "POST /api/iep/evaluations: TEACHER assigned to the learner can create a draft",
  { skip: SKIP },
  async () => {
...
test(
  "POST /api/iep/evaluations: TEACHER without an ACCEPTED assignment is denied (403)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations: PARENT cannot start an evaluation (403)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations: cross-tenant DISTRICT_ADMIN cannot create on another tenant's learner",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "GET /api/iep/evaluations/learner/:learnerId: PARENT sees only submitted/decided evaluations and only the parent-summary fields",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      await insertEval(db, f.learnerAId, f.tenantAId, f.teacherAId, "submitted", {
        ...
        referralReason: "should-not-leak",
      });
      ...
      const res = await app.inject({
        method: "GET",
        url: `/api/iep/evaluations/learner/${f.learnerAId}`,
...
test(
  "GET /api/iep/evaluations/:id: cross-tenant DISTRICT_ADMIN gets 403",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      const res = await app.inject({
        method: "GET",
        url: `/api/iep/evaluations/${row.id}`,
        headers: { Authorization: `Bearer ${token}` },
      });
      assert.equal(res.statusCode, 403);
    } finally {
...
test(
  "GET /api/iep/evaluations/:id: PLATFORM_ADMIN can read across tenants",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test("GET /api/iep/evaluations/:id: invalid UUID returns 400", { skip: SKIP }, async () => {
  const { app, db } = await bootstrap();
  const f = await seed(db);
  try {
    const token = await tokenFor(f.teacherAId, "TEACHER", f.tenantAId);
    const res = await app.inject({
      method: "GET",
      url: "/api/iep/evaluations/not-a-uuid",
      headers: { Authorization: `Bearer ${token}` },
    });
...
test(
  "GET /api/iep/evaluations/:id: PARENT cannot view a draft evaluation (returns 404)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "PATCH /api/iep/evaluations/:id: editing a decided evaluation returns 409",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      const row = await insertEval(
        db,
        f.learnerAId,
        f.tenantAId,
        f.teacherAId,
        "eligibility_determined",
        { decisionEligible: "eligible", decidedAt: new Date() },
      );
      const token = await tokenFor(f.teacherAId, "TEACHER", f.tenantAId);
      const res = await app.inject({
        method: "PATCH",
...
test(
  "PATCH /api/iep/evaluations/:id: TEACHER assigned to the learner can update a draft",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations/:id/submit: only callable from draft (409 otherwise)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      const res = await app.inject({
        method: "POST",
        url: `/api/iep/evaluations/${row.id}/submit`,
        headers: { Authorization: `Bearer ${token}` },
      });
      assert.equal(res.statusCode, 409, "double-submit must return 409, not advance the timestamp");
    } finally {
      await cleanup(db, f);
      await teardown(app, db);
    }
...
test(
  "POST /api/iep/evaluations/:id/submit: happy path moves draft to submitted and stamps submittedAt",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations/:id/decision: rejects from non-submitted statuses with 409",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      const res = await app.inject({
        method: "POST",
        url: `/api/iep/evaluations/${row.id}/decision`,
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
        payload: { decision: "eligible", categories: ["specific_learning_disability"] },
      });
      assert.equal(res.statusCode, 409, "decision before submit must be rejected with 409");
    } finally {
      await cleanup(db, f);
      await teardown(app, db);
    }
...
test(
  "POST /api/iep/evaluations/:id/decision: filters out non-IDEA-13 category strings before persisting",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations/:id/decision: missing decision field returns 400",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "POST /api/iep/evaluations/:id/suggest: AI-service failure surfaces as 502",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    ...
    try {
      const row = await insertEval(db, f.learnerAId, f.tenantAId, f.teacherAId, "draft", {
        referralReason: "concern",
        observations: "...",
      });
      const token = await tokenFor(f.teacherAId, "TEACHER", f.tenantAId);
      const res = await app.inject({
        method: "POST",
        url: `/api/iep/evaluations/${row.id}/suggest`,
        headers: { Authorization: `Bearer ${token}` },
      });
      assert.equal(
        res.statusCode,
...
test(
  "POST /api/iep/evaluations/:id/suggest: persists the AI suggestion when the AI service responds",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
