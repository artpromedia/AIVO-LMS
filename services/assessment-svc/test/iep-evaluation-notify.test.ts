...
async function seed(db: any): Promise<Fixture> {
  ...
  const [t] = await db
    .insert(tenants)
    .values({
      name: `notify-eval-${stamp}`,
      type: "B2B_DISTRICT",
    } as any)
    .returning();
  const [tOther] = await db
    .insert(tenants)
    .values({
      name: `notify-eval-other-${stamp}`,
      type: "B2B_DISTRICT",
    } as any)
    .returning();
...
test(
  "submit dispatches exactly one parent + each district admin email + one in-app, and is idempotent under retry",
  { skip: SKIP },
  async () => {
    ...
    try {
      ...
      const parentEmailCalls = spy.calls.filter(
        (c) => c.url.includes("/iep-notify") && c.body?.template === "evaluation_submitted",
      );
      const adminEmailCalls = spy.calls.filter(
        (c) => c.url.includes("/iep-notify") && c.body?.template === "evaluation_submitted_admin",
      );
      const inAppCalls = spy.calls.filter(
        (c) => c.url.includes("/in-app-notify") && c.body?.template === "evaluation_submitted",
      );
      assert.equal(
        parentEmailCalls.length,
        1,
      ...
      const otherAdminEmail = (
        await db
          .select({ email: (await import("@aivo/db")).users.email })
          .from((await import("@aivo/db")).users)
          .where(
            (await import("drizzle-orm")).eq(
              (await import("@aivo/db")).users.id,
              f.otherTenantAdminId,
            ),
          )
      )[0]?.email;
      assert.ok(
        !adminTos.includes(otherAdminEmail),
      ...
      await settle();
      const afterRetryParent = spy.calls.filter(
        (c) => c.url.includes("/iep-notify") && c.body?.template === "evaluation_submitted",
      ).length;
      assert.equal(
        afterRetryParent,
...
test(
  "decision dispatches exactly one parent email + one in-app notification, and is idempotent",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    ...
    try {
      ...
      const parentDecidedEmails = spy.calls.filter(
        (c) => c.url.includes("/iep-notify") && c.body?.template === "evaluation_decided",
      );
      const parentDecidedInApp = spy.calls.filter(
        (c) => c.url.includes("/in-app-notify") && c.body?.template === "evaluation_decided",
      );
      assert.equal(
        parentDecidedEmails.length,
        1,
        "exactly one decision email must reach the parent",
      );
      assert.equal(
        parentDecidedInApp.length,
        1,
