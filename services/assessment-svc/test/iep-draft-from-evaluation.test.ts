...
async function cleanup(db: any, f: Fixture) {
  ...
  await db
    ...
    .where(inArray(iepEvaluations.id, [f.evalEligibleId, f.evalIneligibleId]));
...
test(
  "create-draft: seeds disability categories, back-link, and present-levels from an eligible evaluation",
  { skip: SKIP },
  async () => {
...
test("create-draft: rejects seeding from a not-eligible evaluation", { skip: SKIP }, async () => {
  const { app, db } = await bootstrap();
  const f = await seed(db);
  try {
    const token = await tokenFor(f.teacherId, "TEACHER", f.tenantId);
...
test(
  "create-draft: works without an evaluation (manual draft path is unchanged)",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    try {
      ...
      const plops = await db
        ...
        .from(iepPresentLevels)
        .where(eq(iepPresentLevels.iepProfileId, profile.id));
      assert.equal(plops.length, 0, "manual drafts must not auto-create present-levels rows");
    } finally {
      await cleanup(db, f);
      await teardown(app, db);
    }
