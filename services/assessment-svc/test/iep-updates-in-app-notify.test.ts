...
async function cleanup(db: any, f: Fixture) {
  ...
  await db
    .delete(parentNotificationPreferences)
...
test(
  "posting a parent-visible progress note creates an in-app notification when inApp=true",
  { skip: SKIP },
  async () => {
...
test(
  "posting a parent-visible note does NOT create an in-app notification when inApp=false",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
...
test(
  "team-only and internal-visibility notes never trigger any parent notification",
  { skip: SKIP },
  async () => {
    const { app, db } = await bootstrap();
    const f = await seed(db);
    ...
    try {
      ...
      const dispatched = recorder.calls.filter(
        (c) =>
...
test("when both channels are off, no notification is sent at all", { skip: SKIP }, async () => {
  ...
  try {
    ...
    assert.equal(dispatched.length, 0, "with both prefs off, neither channel may fire");
  } finally {
    recorder.restore();
    await cleanup(db, f);
    await teardown(app, db);
