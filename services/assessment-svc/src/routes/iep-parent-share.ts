...
async function buildPublicSnapshot(db: any, profile: any) {
  ...
  const [plops, services, goals] = await Promise.all([
    db
      .select({
        area: iepPresentLevels.area,
        narrative: iepPresentLevels.narrative,
      })
    ...
    db
      .select({
        goalText: iepGoals.goalText,
        domain: iepGoals.domain,
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const [updated] = await db
        .update(iepParentShareLinks)
        .set({ revokedAt: new Date() })
        .where(and(eq(iepParentShareLinks.id, linkId), eq(iepParentShareLinks.iepProfileId, id)))
        .returning();
