...
async function buildSectionSnapshot(db: any, profileId: string, section: string): Promise<unknown> {
  ...
  if (section === "plop") {
    return await db
      ...
      .where(eq(iepPresentLevels.iepProfileId, profileId));
  }
  if (section === "profile") {
...
export async function registerIepAuthoringRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (body.fromEvaluationId) {
        if (!isUuid(body.fromEvaluationId))
          return reply.code(400).send({ error: "Invalid fromEvaluationId" });
        const [row] = await db
          .select()
          .from(iepEvaluations)
          .where(eq(iepEvaluations.id, body.fromEvaluationId));
        ...
        if (row.decisionEligible !== "eligible") {
          return reply
            .code(400)
            .send({ error: "Can only seed draft from an eligible determination" });
        }
        evalRow = row;
      }
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!(await canReadSummary(db, claims, learnerId)))
        return reply.code(403).send({ error: "Forbidden" });
      const rows = await db
        .select()
        .from(iepProfiles)
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (profile.source !== "authored")
        return reply.code(404).send({ error: "Not an authored draft" });
      if (!(await canRead(db, claims, profile.learnerId)))
        return reply.code(403).send({ error: "Forbidden" });
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      if (profile.source !== "authored")
        return reply.code(404).send({ error: "Not an authored draft" });
      if (!(await canWrite(db, claims, profile.learnerId)))
        return reply.code(403).send({ error: "Forbidden" });
      ...
      void snapshotSection(
        db,
        id,
        "profile",
        await buildSectionSnapshot(db, id, "profile"),
        claims.sub,
      );
      return updated;
    },
  ...
  app.put(
    ...
    async (req, reply) => {
      ...
      const existing = await db
        .select()
        .from(iepPresentLevels)
        .where(and(eq(iepPresentLevels.iepProfileId, id), eq(iepPresentLevels.area, area)));
      if (existing.length > 0) {
        const [u] = await db
          .update(iepPresentLevels)
          .set({ narrative, updatedAt: new Date() })
          .where(eq(iepPresentLevels.id, existing[0].id))
          .returning();
        void snapshotSection(
          db,
          id,
          "plop",
          await buildSectionSnapshot(db, id, "plop"),
          claims.sub,
        );
        return u;
      }
      const [u] = await db
        .insert(iepPresentLevels)
        .values({
          iepProfileId: id,
          area,
          narrative,
        ...
        .returning();
      void snapshotSection(db, id, "plop", await buildSectionSnapshot(db, id, "plop"), claims.sub);
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      void snapshotSection(
        ...
        id,
        "goals",
        await buildSectionSnapshot(db, id, "goals"),
        claims.sub,
      );
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (!(await canWrite(db, claims, profile.learnerId)))
        return reply.code(403).send({ error: "Forbidden" });
      if (!isEditable(profile)) return reply.code(409).send({ error: "Draft is not editable" });
      await db.delete(iepGoals).where(and(eq(iepGoals.id, goalId), eq(iepGoals.iepProfileId, id)));
      void snapshotSection(
        db,
        id,
        "goals",
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      void snapshotSection(
        ...
        id,
        "services",
        await buildSectionSnapshot(db, id, "services"),
        claims.sub,
      );
    ...
    async (req, reply) => {
      ...
      const [u] = await db
        ...
        .where(
          and(
            ...
            inArray(iepProfiles.lifecycleState, TRANSITIONABLE_LIFECYCLE),
          ),
        )
        .returning();
      ...
      if (to === "in_review") {
        const [learner] = await db
          .select()
          .from(learners)
          .where(eq(learners.id, profile.learnerId));
        if (learner?.parentId) {
          try {
            await db.insert(iepTeamMembers).values({
              iepProfileId: id,
