...
import {
  getIepDraftsByIdNotesSchema,
  getIepDraftsByIdReportsSchema,
  iepDraftsByIdReportsByRidSendSchema,
  getIepDraftsByIdAmendmentsSchema,
  getIepLearnersByLearnerIdTimelineSchema,
  getIepPreferencesSchema,
  getIepInternalUnreadParentUpdatesSchema,
  iepInternalRunReviewRemindersSchema,
  getIepDraftsByIdReportsSeedSchema,
} from "./schemas.js";
...
async function getActiveProfileForLearner(db: any, learnerId: string) {
  ...
  const rows = await db
    ...
    .limit(5);
  return (
    rows.find((r: any) => r.lifecycleState === "finalised") ||
...
async function bestEffortSendEmail(template: string, to: string, data: Record<string, unknown>) {
  try {
    ...
    if (!res.ok) {
      logger.warn("iep-notify email dispatch returned non-2xx", {
        template,
        to,
        status: res.status,
      });
...
export async function checkReviewReminders(db: any): Promise<number> {
  ...
  for (const threshold of [90, 60, 30]) {
    ...
    for (const p of profiles) {
      ...
      const caseManagerRows = await db
        .select({ email: users.email, name: users.name })
        .from(iepTeamMembers)
        .innerJoin(users, eq(users.id, iepTeamMembers.userId))
        .where(and(eq(iepTeamMembers.iepProfileId, p.id), eq(iepTeamMembers.role, "case_manager")));
...
export async function registerIepUpdatesRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    { schema: getIepDraftsByIdNotesSchema },
    async (req, reply) => {
      ...
      const filtered =
        isParent && claims.role !== "PLATFORM_ADMIN"
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const [row] = await db
        ...
        .returning();
      if (vis === "parent") {
        await notifyParentIfOptedIn(db, profile.learnerId, "progress_notes", "iep_progress_note", {
  ...
  app.get(
    ...
    { schema: getIepDraftsByIdReportsSchema },
    async (req, reply) => {
      const claims = await authenticate(req, reply);
      if (!claims) return;
      const { id } = req.params as any;
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const [row] = await db
        .update(iepProgressReports)
        .set(patch)
        .where(eq(iepProgressReports.id, rid))
        .returning();
      return row;
    },
  );
  ...
  app.post(
    ...
    { schema: iepDraftsByIdReportsByRidSendSchema },
    async (req, reply) => {
      const claims = await authenticate(req, reply);
      if (!claims) return;
      const { id, rid } = req.params as any;
  ...
  app.get(
    ...
    { schema: getIepDraftsByIdAmendmentsSchema },
    async (req, reply) => {
      const claims = await authenticate(req, reply);
      if (!claims) return;
      const { id } = req.params as any;
      ...
      const isParent = !!learner && learner.parentId === claims.sub;
      if (
        !(await isTeamMember(db, id, claims.sub)) &&
        !isParent &&
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const currentGoals = await db.select().from(iepGoals).where(eq(iepGoals.iepProfileId, id));
      ...
      const [row] = await db
        ...
        .returning();
      await notifyParentIfOptedIn(db, profile.learnerId, "amendments", "iep_amendment_proposed", {
        summary,
      });
      return row;
    },
    ...
    async (req, reply) => {
      ...
      const [row] = await db
        ...
        .set({
          ...
          revisionCounter:
            response === "acknowledged" ? (a.revisionCounter || 0) + 1 : a.revisionCounter,
          updatedAt: nowTs,
        })
      ...
      if (response === "acknowledged") {
        ...
        if (proposed.profile && typeof proposed.profile === "object") {
          ...
          if (typeof p.communicationSystem === "string")
            patch.communicationSystem = p.communicationSystem;
          ...
          if (Array.isArray(g.remove)) {
            for (const rid of g.remove) {
              ...
              try {
                await db
                  .update(iepGoals)
                  .set({ status: "archived", updatedAt: nowTs })
        ...
        try {
          const [profileNow] = await db
            .select()
            .from(iepProfiles)
            .where(eq(iepProfiles.id, a.iepProfileId));
          const goalsNow = await db
            .select()
            .from(iepGoals)
  ...
  app.get(
    ...
    { schema: getIepLearnersByLearnerIdTimelineSchema },
    async (req, reply) => {
      const claims = await authenticate(req, reply);
      if (!claims) return;
      ...
      const sameTenant = claims.role === "PLATFORM_ADMIN" || learner.tenantId === claims.tenantId;
      const profile = await getActiveProfileForLearner(db, learnerId);
      const teamMember =
        profile && sameTenant ? await isTeamMember(db, profile.id, claims.sub) : false;
      const districtAdmin = sameTenant && claims.role === "DISTRICT_ADMIN";
      const allowed = isParent || claims.role === "PLATFORM_ADMIN" || teamMember || districtAdmin;
      ...
      const districtOnly =
        !isParent && !teamMember && districtAdmin && claims.role !== "PLATFORM_ADMIN";
      ...
      const [notes, reports, amendments, reminders] = await Promise.all([
        ...
        db
          .select({
            id: iepProgressReports.id,
            period: iepProgressReports.period,
            narrative: iepProgressReports.narrative,
            status: iepProgressReports.status,
            sentAt: iepProgressReports.sentAt,
            sentBy: iepProgressReports.sentBy,
            sentByName: users.name,
            createdAt: iepProgressReports.createdAt,
          })
          .from(iepProgressReports)
        ...
        db
          .select({
            id: iepAmendments.id,
            summary: iepAmendments.summary,
            proposedChanges: iepAmendments.proposedChanges,
            status: iepAmendments.status,
            proposedBy: iepAmendments.proposedBy,
  ...
  app.get(
    ...
    { schema: getIepInternalUnreadParentUpdatesSchema },
    async (req, reply) => {
      ...
      const profileRows = await db
        ...
        .where(and(eq(learners.tenantId, tenantId), eq(iepProfiles.lifecycleState, "finalised")));
      const profileIds = profileRows.map((p: any) => p.id);
      if (profileIds.length === 0) return { count: 0 };
      const [notesC] = await db
      ...
      return {
        count:
          Number(notesC?.count || 0) + Number(reportsC?.count || 0) + Number(amendsC?.count || 0),
      };
    },
  );
  ...
  app.post(
    ...
    { schema: iepInternalRunReviewRemindersSchema },
    async (req, reply) => {
      const internalKey = req.headers["x-internal-key"];
      if (!internalKey || internalKey !== INTERNAL_KEY) {
        return reply.code(401).send({ error: "Unauthorized" });
  ...
  app.get(
    ...
    { schema: getIepDraftsByIdReportsSeedSchema },
    async (req, reply) => {
      const claims = await authenticate(req, reply);
      if (!claims) return;
      const { id } = req.params as any;
      ...
      const goals = await db
        .select()
        .from(iepGoals)
        .where(eq(iepGoals.learnerId, profile.learnerId));
      if (goals.length === 0) {
        return { seeded: false, narrative: "" };
      }
      const lines: string[] = [];
      lines.push(
        `Quarterly progress summary across ${goals.length} active goal${goals.length === 1 ? "" : "s"}:`,
      );
      for (const g of goals.slice(0, 8)) {
        ...
        lines.push(
          `• ${g.domain || "Goal"}: ${g.goalText.slice(0, 140)} — baseline ${baseline}, target ${target} (${status}).`,
        );
      }
      lines.push("");
      lines.push(
        "Edit this draft to add classroom observations, work samples, and next-quarter focus before sending.",
      );
