...
import { teacherAssessments, learners, learnerTeachers } from "@aivo/db";
...
export async function registerTeacherAssessmentRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const { allowed, tenantId } = await canSubmitTeacherAssessment(db, claims, body.learnerId);
      if (!allowed || !tenantId) {
        return reply.status(403).send({
          error:
            "Only an assigned teacher, SPED lead, or admin may submit a teacher assessment for this learner",
        });
      }
