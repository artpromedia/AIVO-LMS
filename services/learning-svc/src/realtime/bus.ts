/**
 * Cross-replica pub/sub bus for live-fan-out surfaces — learning-svc copy.
 *
 * Mirrors `services/comms-svc/src/realtime/bus.ts` so each service can own
 * its own runtime/connection without a new shared package. Subjects are
 * platform-wide (a parent on web subscribing to `session.coview.<learnerId>`
 * gets events whether they originated in learning-svc or comms-svc, as long
 * as both speak to the same NATS).
 *
 * Subjects this service publishes:
 *   session.coview.<learnerId>          — lesson-session row created/updated
 *
 * See the comms-svc copy for the full rationale and lazy-NATS pattern.
 */
