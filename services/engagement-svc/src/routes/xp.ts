...
import {
  xpAwardSchema,
  getProfileByLearnerIdSchema,
  streakUpdateSchema,
  streakFreezeSchema,
  badgeAwardSchema,
  getLeaderboardByScopeSchema,
  getCurrencyByLearnerIdSchema,
} from "./schemas.js";
...
const BADGE_RULES: {
  ...
  name: string;
  rarity: string;
  check: (totalXp: number, level: number, eventType: string, eventCount: number) => boolean;
}[] = [
  {
    type: "first_steps",
    name: "First Steps",
    rarity: "common",
    check: (_xp, _lvl, et) => et === "first_lesson",
  },
  ...
  { type: "xp_10000", name: "XP Legend", rarity: "legendary", check: (xp) => xp >= 10000 },
  {
    ...
    name: "Perfectionist",
    rarity: "rare",
    check: (_xp, _lvl, et) => et === "perfect_score",
  },
  {
    type: "homework_hero",
    name: "Homework Hero",
    rarity: "common",
    check: (_xp, _lvl, et) => et === "homework_completed",
  },
  {
    type: "quest_master",
    name: "Quest Master",
    rarity: "epic",
    check: (_xp, _lvl, et) => et === "quest_completed",
  },
  {
    type: "streak_7",
    name: "Week Warrior",
    rarity: "rare",
    check: (_xp, _lvl, et) => et === "streak_maintained",
  },
  {
    type: "challenge_champ",
    name: "Challenge Champion",
    rarity: "epic",
    check: (_xp, _lvl, et) => et === "challenge_won",
  },
];
...
export function registerXpRoutes(
  ...
  app.post("/api/engagement/xp/award", { schema: xpAwardSchema }, async (request, reply) => {
    ...
    if (!XP_EVENT_TYPES.includes(body.eventType as any)) {
      return reply
        .status(400)
        .send({ error: `Invalid eventType. Must be one of: ${XP_EVENT_TYPES.join(", ")}` });
    }
    ...
    if (
      body.coinReward !== undefined &&
      (typeof body.coinReward !== "number" || body.coinReward < 0 || body.coinReward > 500)
    ) {
      return reply.status(400).send({ error: "coinReward must be 0-500" });
    }

    if (
      body.gemReward !== undefined &&
      (typeof body.gemReward !== "number" || body.gemReward < 0 || body.gemReward > 100)
    ) {
  ...
  app.get(
    ...
    { schema: getProfileByLearnerIdSchema },
    async (request, reply) => {
      const claims = await authenticateRequest(request, reply);
      if (!claims) return;
      ...
      return {
        ...
        xpProgress:
          totalXp -
          (level > 1
            ? Array.from({ length: level - 1 }, (_, i) => (i + 1) * (i + 1) * 100).reduce(
                (a, b) => a + b,
                0,
              )
            : 0),
  ...
  app.post("/api/engagement/badge/award", { schema: badgeAwardSchema }, async (request, reply) => {
    ...
    const existing = await db.select().from(badges).where(eq(badges.learnerId, body.learnerId));
  ...
  app.get(
    ...
    { schema: getLeaderboardByScopeSchema },
    async (request, reply) => {
      const claims = await authenticateRequest(request, reply);
      if (!claims) return;
      ...
      if (scope === "class") {
        if (!rawScopeId) {
          return reply
            .status(400)
            .send({ error: "scopeId (classroom id) is required for class scope" });
        }
        scopeId = rawScopeId;
        ...
        if (rawScopeId) {
          if (!isAdmin && rawScopeId !== callerTenantId) {
            return reply
              .status(403)
              .send({ error: "Cannot read leaderboard for a different tenant" });
          }
          scopeId = rawScopeId;
          ...
          return reply
            .status(400)
            .send({ error: "scopeId is required for tenant scope when caller has no tenant" });
        }
  ...
  app.get(
    ...
    { schema: getCurrencyByLearnerIdSchema },
    async (request, reply) => {
      const claims = await authenticateRequest(request, reply);
      if (!claims) return;
