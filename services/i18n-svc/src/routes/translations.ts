...
export function registerTranslationRoutes(app: FastifyInstance, db: any) {
  ...
  app.get(
    ...
    { schema: getTranslationsSchema },
    async (request, reply) => {
      const { locale } = request.params as any;
      if (!SUPPORTED_LOCALES.includes(locale)) {
        return reply
          .code(404)
          .send({ error: "Locale not supported", supported: SUPPORTED_LOCALES });
      }
      return {
        locale,
  ...
  app.get(
    ...
    { schema: getNamespaceTranslationsSchema },
    async (request, reply) => {
  ...
  app.get(
    ...
    { schema: getLocalizedContentSchema },
    async (request, reply) => {
