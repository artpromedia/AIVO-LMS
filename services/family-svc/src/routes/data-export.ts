...
import {
  ...
  iepProfiles,
  iepGoals,
  learnerFunctioningLevels,
  brainStates,
  brainStateSnapshots,
  brainRecommendations,
  functionalMilestones,
  parentReportedEvents,
  lessonSessions,
  gradebookEntries,
  tutorSessions,
  homeworkAssignments,
...
export async function registerDataExportRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    { schema: getDataExportByLearnerIdSchema },
    async (req, reply) => {
      ...
      const [functioning] = await db
        .select()
        .from(learnerFunctioningLevels)
      ...
      const snapshots = await db
        ...
        .where(eq(brainStateSnapshots.learnerId, learnerId));
      ...
      const milestones = await db
        .select()
        .from(functionalMilestones)
        .where(eq(functionalMilestones.learnerId, learnerId));
      const events = await db
        .select()
        .from(parentReportedEvents)
        .where(eq(parentReportedEvents.learnerId, learnerId));
