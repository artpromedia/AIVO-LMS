...
describe("OneRoster REST connector", () => {
  ...
  it("extracts a full snapshot across endpoints", async () => {
    const byPath: Record<string, unknown> = {
      ...
      "/enrollments": { enrollments: [{ sourcedId: "e1", class: { sourcedId: "cl1" }, user: { sourcedId: "u1" }, school: { sourcedId: "o1" }, role: "student" }] },
    };
...
describe("ClassLink connector (OneRoster wrapper)", () => {
  it("retags the snapshot provider as classlink", async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      ...
      if (url.includes("/orgs")) return jsonResponse({ orgs: [{ sourcedId: "o1", name: "D", type: "district" }] });
      return jsonResponse({ [Object.keys({})[0] ?? "x"]: [] });
    });
    const snap = await extractClassLink(
      { baseUrl: "https://cl/v1p1", tokenUrl: "https://cl/token", clientId: "i", clientSecret: "s", pageSize: 100 },
...
describe("Clever connector", () => {
  it("maps a user's role map to canonical roles", () => {
    const u = mapCleverUser({ id: "u1", name: { first: "A", last: "B" }, roles: { teacher: {} }, school: "sch-1" });
    expect(u.primaryRole).toBe("teacher");
    expect(u.orgSourcedIds).toEqual(["sch-1"]);
  });
  ...
  it("extracts a snapshot with paged cursors", async () => {
    const data = {
      ...
      "/v3.0/terms": [],
      "/v3.0/courses": [],
      "/v3.0/users": [{ data: { id: "u1", name: { first: "A", last: "B" }, roles: { student: {} }, school: "sch-1" } }],
      "/v3.0/sections": [{ data: { id: "sec-1", name: "M", school: "sch-1", teacher: "t1", students: ["u1"] } }],
    } as Record<string, unknown>;
    const fetchImpl = vi.fn(async (url: string) => {
      const key = Object.keys(data).find((k) => url.includes(k))!;
      return jsonResponse({ data: data[key] });
    });
    const snap = await extractClever({ token: "tok", pageSize: 1000 }, fetchImpl as unknown as typeof fetch);
    expect(snap.orgs).toHaveLength(1);
