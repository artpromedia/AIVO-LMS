...
async function fetchConnectedLearners(db: any, userId: string): Promise<any[]> {
  const [careRows, teachRows, theraRows] = await Promise.all([
    ...
    db
      ...
      .where(
        and(eq(learnerTeachers.teacherUserId, userId), eq(learnerTeachers.status, "ACCEPTED")),
      ),
    db
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const results = await db
          .select()
          .from(learners)
          .where(eq(learners.tenantId, user.tenantId));
        return results;
      } catch (err) {
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const [tenantRow] = await db
          ...
          .limit(1);
        const tier =
          tenantRow?.licensingTier ||
          (tenantRow?.type === "B2C_FAMILY" ? "B2C_PARENT_PAY" : "B2B_SEAT_LICENSED");
        if (tier === "B2C_PARENT_PAY") {
          const existingForParent = await db
            .select({ id: learners.id })
            .from(learners)
            .where(eq(learners.parentId, parentUserId))
            .limit(2);
          if (existingForParent.length >= 1) {
            return reply.status(409).send({
              error:
                "B2C accounts are limited to one learner. Upgrade to a school plan to add more.",
              licensingTier: tier,
            });
          }
        ...
        try {
          ...
          app.log.warn(
            { err: insertErr, userSub: user?.sub },
            "Learner extended insert failed; retrying minimal insert",
          );
          [learner] = await db
            .insert(learners)
            .values({
