...
test(
  "verify-mfa rejects email OTP when issued mfaToken is bound to webauthn/totp",
  { skip: SKIP },
  async () => {
    ...
    const fakeWebauthnToken = [
      ...
      Buffer.from(
        JSON.stringify({ sub: "00000000-0000-0000-0000-000000000000", mfaMethod: "webauthn" }),
      ).toString("base64url"),
      "sig",
    ].join(".");
...
test(
  "resend-mfa refuses to send email OTP when token is bound to webauthn/totp",
  { skip: SKIP },
  async () => {
    const fakeTotpToken = [
      "eyJhbGciOiJIUzI1NiJ9",
      Buffer.from(
        JSON.stringify({ sub: "00000000-0000-0000-0000-000000000000", mfaMethod: "totp" }),
      ).toString("base64url"),
      "sig",
    ].join(".");
    ...
    assert.ok(status === 400 || status === 401 || status === 403, `expected 4xx, got ${status}`);
  },
);
