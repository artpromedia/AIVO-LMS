...
test(
  "API keys: create returns plaintext exactly once and stores argon2 hash",
  { skip: SKIP },
  async () => {
...
test(
  "API keys: rotate keeps old key working through grace, then revoke kills it",
  { skip: SKIP },
  async () => {
    const { app, db, auth, adminId, validateApiKey, closeDb } = await bootstrap();
    try {
      ...
      const rot = await app.inject({
        method: "POST",
        url: `/api/admin-svc/api-keys/${original.key.id}/rotate`,
        headers: auth,
      });
      ...
      const rev = await app.inject({
        method: "POST",
        url: `/api/admin-svc/api-keys/${rotated.key.id}/revoke`,
        headers: auth,
      });
      ...
      await db
        .delete(apiKeys)
        .where(or(eq(apiKeys.id, original.key.id), eq(apiKeys.id, rotated.key.id)));
    } finally {
      await cleanupAdmin(db, adminId);
      await app.close();
      await closeDb(db);
...
test("API keys: rotate refuses when already revoked", { skip: SKIP }, async () => {
  ...
  try {
    ...
    await app.inject({
      method: "POST",
      url: `/api/admin-svc/api-keys/${k.key.id}/revoke`,
      headers: auth,
    });
    const rot = await app.inject({
      method: "POST",
      url: `/api/admin-svc/api-keys/${k.key.id}/rotate`,
      headers: auth,
    });
