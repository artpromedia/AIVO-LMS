...
import {
  getAdminSvcApiKeysSchema,
  adminSvcApiKeysSchema,
  adminSvcApiKeysByIdRotateSchema,
  adminSvcApiKeysByIdRevokeSchema,
} from "./schemas.js";
...
export function registerApiKeyRoutes(app: FastifyInstance, db: any) {
  app.get(
    ...
    { schema: getAdminSvcApiKeysSchema, preHandler: requirePlatformAdmin },
    async () => {
      const rows = await db
        .select({
          id: apiKeys.id,
  ...
  app.post(
    ...
    { schema: adminSvcApiKeysByIdRotateSchema, preHandler: requirePlatformAdmin },
    async (req: any, reply) => {
      ...
      if (existing.revokedAt)
        return reply.status(400).send({ error: "Cannot rotate a revoked key" });
      ...
      await logKeyAction(
        ...
        req.user,
        created,
        { rotatedFromId: existing.id, graceHours: ROTATION_GRACE_HOURS },
        req,
      );
  ...
  app.post(
    ...
    { schema: adminSvcApiKeysByIdRevokeSchema, preHandler: requirePlatformAdmin },
    async (req: any, reply) => {
      const { id } = req.params as { id: string };
      const [existing] = await db.select().from(apiKeys).where(eq(apiKeys.id, id)).limit(1);
      if (!existing) return reply.status(404).send({ error: "Key not found" });
