...
The `Marketing Site` workflow runs `scripts/start-marketing.sh`, which binds Next.js's Turbopack dev server **directly to port 3003** (`next dev --port 3003 --turbopack --hostname 0.0.0.0`). Replit's workflow supervisor probes the **local** port a process actually opens, and only recognizes ports declared as `localPort` in `.replit` (mapped `3003 -> 3003`, matching the working Web App=5000 / Identity=3001 / Brain=3002 pattern). Earlier attempts that bound 5173/8000 — values that exist only as externalPort targets, not declared localPorts — failed with `DIDNT_OPEN_A_PORT` even though the dev server compiled `/` and served HTTP 200 (the in-script curl warmup loop confirmed it), because the supervisor was watching a port the process never opened locally. `waitForPort = 3003` now matches what the process actually opens, so the workflow card stays green. Do not reintroduce a `localPort != waitForPort` mapping for this workflow.

The start script also (a) builds `@aivo/brand` tokens first (the `@import "@aivo/brand/tokens.css"` in `app/layout.tsx` resolves to a dist file that must exist before compile) and (b) kills any stray `next-server` child whose cwd is `apps/marketing` before binding, because Turbopack forks a `next-server` child that can outlive the parent and keep 3003 bound, making the next restart die with `EADDRINUSE: 0.0.0.0:3003`. Neither `fuser` nor `ss` is in this image, so strays are found by matching `/proc/<pid>/cwd` and scoped to `apps/marketing` so the Web App's own `next-server` (cwd = `apps/web-v2`) is never touched.

**Port-collision fix (assessment-svc 3003 → 3012):** `assessment-svc` was historically pinned to 3003 in `scripts/start-services.sh`, the exact port the Marketing Site needs. On a fresh boot the backend group started first and grabbed 3003, so the Marketing Site workflow died with `EADDRINUSE`. Because `.replit` ports are platform-managed (both `edit` and `bash` reject writes to `.replit`, and no port-management tool is exposed), marketing **must** stay on 3003 — so assessment-svc was moved to **3012** (confirmed free; no env override pins it) instead. All consumers were updated to match: `scripts/start-services.sh`, `services/assessment-svc/src/index.ts`, `services/identity-svc/src/routes/district.ts`, `services/status-page-svc/src/routes/status.ts` + `ops-alerts-outbox.ts`, `scripts/smoke/iep-integration-smoke.sh`, and `apps/mobile/constants/api.ts`. (`apps/web-v2` already defaulted `ASSESSMENT_SVC_URL` to 3071 and is unaffected.) After moving it, the boot order is: restart **Identity Service** (frees 3003, binds assessment on 3012) **then** restart **Marketing Site** (binds 3003). Do not move assessment-svc back to 3003.
