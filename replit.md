...
The `Marketing Site` workflow runs `scripts/start-marketing.sh`, which binds Next.js's Turbopack dev server **directly to port 5173** (`next dev --port 5173 --hostname 0.0.0.0`). Earlier the script bound to localPort 3004 with a `localPort = 3004 / externalPort = 5173` mapping in `.replit`, but Replit's workflow supervisor probes the **local** port a process actually opens — not the external-port mapping — so even though the dev server compiled `/` and served HTTP 200 in ~2s (and the in-script curl warmup loop confirmed it), the supervisor never saw a listener on 5173 locally and killed the workflow with `DIDNT_OPEN_A_PORT`. Binding Next.js to 5173 directly removes the localPort↔externalPort indirection: `waitForPort = 5173` now matches what the process actually opens, the workflow card stays green, and `restart_workflow Marketing Site` returns a healthy state with `openPorts: [5173]`. Do not reintroduce a `3004 -> 5173` (or any other localPort != waitForPort) mapping for this workflow.

Note: `.replit` still contains a stale `[[ports]] localPort = 3004 / externalPort = 5173` entry alongside the new `localPort = 5173 / externalPort = 5173` entry. The stale entry is harmless because nothing binds to 3004 anymore and the supervisor uses the matching `5173 -> 5173` mapping, but `.replit` is platform-managed and could not be edited by the agent that made this change (no port-management tool/callback was exposed; both the `edit` tool and `bash` reject direct writes to `.replit`). Remove the stale `localPort = 3004 / externalPort = 5173` block by hand next time `.replit` is touched.
