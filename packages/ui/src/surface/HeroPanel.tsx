 ...
 */
export interface HeroPanelProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "title"> {
...
const TONE_WASH: Record<HeroPanelProps["tone"] & string, string> = {
  ...
  neutral: "bg-[linear-gradient(180deg,#ffffff_0%,var(--aivo-color-surface-canvas,#f4f6f5)_100%)]",
};

export const HeroPanel = React.forwardRef<HTMLDivElement, HeroPanelProps>(function HeroPanel(
  {
    state = "default",
    tone = "primary",
    eyebrow,
    ...
    >
      <div className="flex flex-col-reverse md:flex-row md:items-center gap-6 md:gap-10">
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          <h1 className="iw-display text-[var(--aivo-color-text-default,#0f172a)]">{title}</h1>
          {subtitle && (
        ...
        {media && <div className="shrink-0 md:w-[280px] md:max-w-[40%]">{media}</div>}
      </div>
    </GlassCard>
  );
});
