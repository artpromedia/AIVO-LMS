...
export interface EmptyStateProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "title"> {
  ...
  tone?: Extract<AivoTone, "neutral" | "primary" | "accent" | "warm" | "info" | "success">;
...
export const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>(function EmptyState(
  ...
  return (
    <div
      ref={ref}
      {...rest}
      className={cn("flex flex-col items-center text-center gap-3 py-10 px-6", className)}
    >
      {Icon && (
        <span
          className={cn(
      ...
      <h3 className="iw-label text-[var(--aivo-color-text-default,#0f172a)]">{title}</h3>
      {body && (
        <p className="text-sm text-[var(--aivo-color-text-muted,#64748b)] max-w-prose">{body}</p>
      )}
