...
const KIND_TINT: Record<StateKind, { ribbon: string; halo: string; label: string }> = {
  blocked_permission: {
    ribbon: "bg-[var(--aivo-color-status-warning-subtle)] text-[var(--aivo-color-status-warning-strong)] border-[var(--aivo-color-status-warning-default)]",
  ...
  blocked_safety: {
    ribbon: "bg-[var(--aivo-color-status-error-subtle)] text-[var(--aivo-color-status-error-strong)] border-[var(--aivo-color-status-error-default)]",
    halo: "bg-[var(--aivo-color-status-error-subtle)] text-[var(--aivo-color-status-error-strong)]",
    label: "Safety hold",
  },
  consent_required: {
    ribbon: "bg-[var(--aivo-color-aivoPurple-50)] text-[var(--aivo-color-aivoPurple-700)] border-[var(--aivo-color-aivoPurple-100)]",
  ...
  alignment_missing: {
    ribbon: "bg-[var(--aivo-color-aivoOrange-50)] text-[var(--aivo-color-aivoOrange-700)] border-[var(--aivo-color-aivoOrange-100)]",
  ...
  offline: {
    ribbon: "bg-[var(--aivo-color-status-info-subtle)] text-[var(--aivo-color-status-info-strong)] border-[var(--aivo-color-status-info-default)]",
...
export function StateCard({
  ...
  return (
    <section
      role={kind === "blocked_safety" || kind === "upload_failed" || kind === "payment_failed" ? "alert" : "status"}
      aria-live={
    ...
    >
      <header className="flex items-start gap-4">
        <span
          className={cn("shrink-0 w-12 h-12 rounded-iw-control flex items-center justify-center", tint.halo)}
          aria-hidden="true"
        >
          {KIND_ICON[kind]}
        </span>
        <div className="flex-1 min-w-0 flex flex-col gap-1.5">
          ...
          <h3 className="text-xl md:text-2xl font-semibold text-iw-text-strong leading-snug">{title}</h3>
          {body ? (
            <p className="text-sm md:text-base text-iw-text-muted leading-relaxed max-w-2xl">{body}</p>
          ) : null}
        </div>
      </header>
      {(action || secondary) && (
      ...
      {support ? (
        <p className="text-xs text-iw-text-muted leading-relaxed">{support}</p>
      ) : null}
    </section>
  );
}
