...
export const SidePanel = React.forwardRef<HTMLDivElement, SidePanelProps>(
  function SidePanel(
    ...
    return (
      <div
        className={cn(
          "fixed inset-0 z-50 flex aivo-motion-lesson-reveal",
          align
        )}
        role="dialog"
        aria-modal="true"
        ...
        >
          <header className="flex items-start justify-between gap-4 p-5 md:p-6 border-b border-[var(--aivo-color-border-subtle,rgba(15,23,42,0.06))]">
            <div className="min-w-0 flex flex-col gap-1">
              ...
              {description && (
                <p className="text-sm text-[var(--aivo-color-text-muted,#64748b)]">
                  {description}
                </p>
              )}
            </div>
          ...
          <div className="flex-1 min-h-0 overflow-auto p-5 md:p-6">
            {children}
          </div>
          {footer && (
