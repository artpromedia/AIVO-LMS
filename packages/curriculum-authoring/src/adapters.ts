...
import type { K8CurriculumPack } from "./schemas.js";

const FRAMEWORK_BY_SUBJECT: Record<K8CurriculumPack["skills"][number]["subject"], SkillGraph["framework"]> = {
  math: "CCSS-Math",
  ela: "CCSS-ELA",
  science: "NGSS",
  writing: "CCSS-ELA",
...
export function toSkillGraphs(pack: K8CurriculumPack): SkillGraph[] {
  ...
  return [...groups.entries()].map(([key, skills]) => {
    const [subject, gradeBand] = key.split(":") as [K8CurriculumPack["skills"][number]["subject"], SkillGraph["gradeBand"]];
    return {
