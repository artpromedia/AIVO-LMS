import {
  K8_CURRICULUM_SCHEMA_VERSION,
  type CurriculumSubject,
  type K8CurriculumPack,
  type ReviewerRole,
} from "./types.js";
  ...
  if (!hasApprovedReview(review)) {
    issues.push(
      issue(
        "not_sme_approved",
        path,
        "Production curriculum content must have SME-approved review metadata.",
      ),
    );
  }
}
  ...
  if (input.schemaVersion !== K8_CURRICULUM_SCHEMA_VERSION) {
    issues.push(
      issue(
        "schema_invalid",
        "schemaVersion",
        `schemaVersion must be ${K8_CURRICULUM_SCHEMA_VERSION}.`,
      ),
    );
  }
  if (!isObject(input.metadata))
    issues.push(issue("schema_invalid", "metadata", "metadata is required."));
  if (!Array.isArray(input.standards))
    issues.push(issue("schema_invalid", "standards", "standards must be an array."));
  if (!Array.isArray(input.skills))
    issues.push(issue("schema_invalid", "skills", "skills must be an array."));
  if (!Array.isArray(input.templates))
    issues.push(issue("schema_invalid", "templates", "templates must be an array."));
...
export function validateK8CurriculumPack(input: unknown): CurriculumPackValidationResult {
  ...
  for (const [i, rawStandard] of pack.standards.entries()) {
    if (
      !isObject(rawStandard) ||
      !isString(rawStandard.code) ||
      !isString(rawStandard.description)
    ) {
      issues.push(
        issue("schema_invalid", `standards.${i}`, "Each standard requires code and description."),
      );
      continue;
    }
    if (standards.has(rawStandard.code))
      issues.push(
        issue(
          "duplicate_standard",
          `standards.${i}.code`,
          `Duplicate standard code ${rawStandard.code}.`,
        ),
      );
  ...
  for (const [i, skill] of pack.skills.entries()) {
    if (
      !isObject(skill) ||
      !isString(skill.id) ||
      !isString(skill.subject) ||
      !SUBJECTS.has(skill.subject) ||
      !isString(skill.gradeBand) ||
      !GRADE_BANDS.has(skill.gradeBand)
    ) {
      issues.push(
        issue(
          "schema_invalid",
          `skills.${i}`,
          "Each skill requires id, supported subject, and K-8 gradeBand.",
        ),
      );
      continue;
    }
    if (skills.has(skill.id))
      issues.push(issue("duplicate_skill", `skills.${i}.id`, `Duplicate skill id ${skill.id}.`));
    ...
    if (!isStringArray(skill.standardCodes)) {
      issues.push(
        issue(
          "schema_invalid",
          `skills.${i}.standardCodes`,
          "Skill standardCodes must be a non-empty string array.",
        ),
      );
    } else {
        ...
        if (!standards.has(code))
          issues.push(
            issue(
              "unknown_standard",
              `skills.${i}.standardCodes`,
              `Skill ${skill.id} references unknown standard ${code}.`,
            ),
          );
      }
    }
  ...
  for (const [i, template] of pack.templates.entries()) {
    if (
      ...
      !isString(template.subject) ||
      !SUBJECTS.has(template.subject) ||
      !isString(template.gradeBand) ||
      !GRADE_BANDS.has(template.gradeBand)
    ) {
      issues.push(
        issue(
          "schema_invalid",
          `templates.${i}`,
          "Each template requires id, supported subject, and K-8 gradeBand.",
        ),
      );
      continue;
    }
    if (templates.has(template.id))
      issues.push(
        issue("duplicate_template", `templates.${i}.id`, `Duplicate template id ${template.id}.`),
      );
    ...
    for (const code of baseStandards) {
      if (typeof code === "string" && !standards.has(code))
        issues.push(
          issue(
            "unknown_standard",
            `templates.${i}.baseStandards`,
            `Template ${template.id} references unknown standard ${code}.`,
          ),
        );
    }
    ...
    for (const [u, unit] of units.entries()) {
      if (!isObject(unit)) {
        issues.push(
          issue(
            "schema_invalid",
            `templates.${i}.unitBlueprints.${u}`,
            "Unit blueprint must be an object.",
          ),
        );
        continue;
      }
      ...
      for (const skillId of unitSkills) {
        ...
        if (!skill) {
          issues.push(
            issue(
              "unknown_skill",
              `templates.${i}.unitBlueprints.${u}.skillIds`,
              `Unit references unknown skill ${skillId}.`,
            ),
          );
          ...
          issues.push(
            issue(
              "skill_subject_mismatch",
              `templates.${i}.unitBlueprints.${u}.skillIds`,
              `Skill ${skillId} is ${skill.subject}, not ${template.subject}.`,
            ),
          );
          ...
          issues.push(
            issue(
              "skill_grade_mismatch",
              `templates.${i}.unitBlueprints.${u}.skillIds`,
              `Skill ${skillId} is gradeBand ${skill.gradeBand}, not ${template.gradeBand}.`,
            ),
          );
        }
      }
      ...
      for (const [l, lesson] of lessons.entries()) {
        if (!isObject(lesson)) {
          issues.push(
            issue(
              "schema_invalid",
              `templates.${i}.unitBlueprints.${u}.lessonTopics.${l}`,
              "Lesson topic must be an object.",
            ),
          );
          continue;
        }
        assertApproved(
          lesson.review,
          `templates.${i}.unitBlueprints.${u}.lessonTopics.${l}.review`,
          issues,
        );
        const lessonSkills = Array.isArray(lesson.skillIds) ? lesson.skillIds : [];
        for (const skillId of lessonSkills) {
          if (typeof skillId === "string" && !skills.has(skillId))
            issues.push(
              issue(
                "unknown_skill",
                `templates.${i}.unitBlueprints.${u}.lessonTopics.${l}.skillIds`,
                `Lesson references unknown skill ${skillId}.`,
              ),
            );
        }
      }
  ...
  for (const [i, item] of pack.itemBankItems.entries()) {
    if (!isObject(item) || !isString(item.id) || !isString(item.skillId)) {
      issues.push(
        issue("schema_invalid", `itemBankItems.${i}`, "Each item requires id and skillId."),
      );
      continue;
    }
    if (itemIds.has(item.id))
      issues.push(
        issue("duplicate_item", `itemBankItems.${i}.id`, `Duplicate item id ${item.id}.`),
      );
    itemIds.add(item.id);
    ...
    if (!skill) {
      issues.push(
        issue(
          "unknown_skill",
          `itemBankItems.${i}.skillId`,
          `Item ${item.id} references unknown skill ${item.skillId}.`,
        ),
      );
    } else {
      if (item.subject !== skill.subject)
        issues.push(
          issue(
            "skill_subject_mismatch",
            `itemBankItems.${i}.subject`,
            `Item ${item.id} subject ${String(item.subject)} does not match skill ${skill.subject}.`,
          ),
        );
      if (item.gradeBand !== skill.gradeBand)
        issues.push(
          issue(
            "skill_grade_mismatch",
            `itemBankItems.${i}.gradeBand`,
            `Item ${item.id} gradeBand ${String(item.gradeBand)} does not match skill ${skill.gradeBand}.`,
          ),
        );
    }
    if (["multiple-choice", "multiple-select", "true-false"].includes(String(item.questionType))) {
      ...
      if (
        choices.length < 2 ||
        !choices.some((choice) => isObject(choice) && choice.isCorrect === true)
      ) {
        issues.push(
          issue(
            "schema_invalid",
            `itemBankItems.${i}.choices`,
            "Selected-response items require at least two choices and one correct answer.",
          ),
        );
      }
    }
    if (
      ["short-answer", "constructed-response"].includes(String(item.questionType)) &&
      !isString(item.scoringRubric)
    ) {
      issues.push(
        issue(
          "schema_invalid",
          `itemBankItems.${i}.scoringRubric`,
          "Open-response items require a scoring rubric.",
        ),
      );
    }
  }
  ...
  for (const [skillId, skill] of skills.entries()) {
    ...
    for (const pre of prereqs) {
      if (typeof pre === "string" && !skills.has(pre))
        issues.push(
          issue(
            "unknown_skill",
            `skills.${skillId}.prerequisiteSkillIds`,
            `Skill ${skillId} references unknown prerequisite ${pre}.`,
          ),
        );
    }
  }
    ...
    if (visiting.has(skillId)) {
      issues.push(
        issue(
          "cycle_detected",
          `skills.${skillId}.prerequisiteSkillIds`,
          `Cycle detected: ${[...path, skillId].join(" -> ")}.`,
        ),
      );
      return;
    }
  ...
  const packReviews = Array.isArray(pack.metadata.expertReviews)
    ? pack.metadata.expertReviews.filter(hasApprovedReview)
    : [];
  const roles = new Set(packReviews.map((review) => review.reviewerRole).filter(Boolean));
  if (!roles.has("sped_specialist"))
    issues.push(
      issue(
        "missing_sped_review",
        "metadata.expertReviews",
        "K-8 curriculum pack requires SPED specialist approval before production import.",
      ),
    );
    ...
    if (!roles.has(requiredRole))
      issues.push(
        issue(
          "missing_subject_review",
          "metadata.expertReviews",
          `Curriculum pack includes ${subject}; requires ${requiredRole} approval.`,
        ),
      );
  }
