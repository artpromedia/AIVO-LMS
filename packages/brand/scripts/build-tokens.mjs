...
// age-tier palettes; this emits the typed module both learner-ui (web)
// and mobile-ui (React Native) import, so the palettes cannot drift.
const tierThemes = JSON.parse(
  fs.readFileSync(path.join(root, "tokens/modes/tier-themes.json"), "utf8"),
).tierThemes;
...
const tierData = Object.fromEntries(
  Object.entries(tierThemes).map(([tier, def]) => [
    tier,
    {
      id: tier,
      name: def.name,
      ...
      colors: stripMeta(def.colors),
      mobile: stripMeta(def.mobile),
      radius: def.radius,
      paceMultiplier: def.paceMultiplier,
    },
  ]),
);
...
fs.writeFileSync(
  path.join(generatedDir, "tier-themes.ts"),
  `// GENERATED by scripts/build-tokens.mjs from tokens/modes/tier-themes.json.
// DO NOT EDIT — change the JSON token source instead.

export type AgeTierId = "EARLY" | "MIDDLE" | "HIGH";
...
export interface TierThemeMobileColors {
  tabBar: string;
  tabBarActive: string;
...
export interface TierThemeData {
  id: AgeTierId;
  name: string;
  tagline: string;
  nameKey: string;
  ...
  mobile: TierThemeMobileColors;
  radius: { sm: number; md: number; lg: number; pill: number };
  paceMultiplier: number;
}

export const TIER_THEME_DATA: Record<AgeTierId, TierThemeData> = ${JSON.stringify(tierData, null, 2)} as const;
`,
);
