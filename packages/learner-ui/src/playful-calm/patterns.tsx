...
export function MascotCoach({ message, muted }: { message: string; muted?: boolean }) {
  return (
    <Card style={{ padding: 16 }}>
      <p>{message}</p>
      <Button variant="audio">{muted ? "Unmute" : "Read aloud"}</Button>
    </Card>
  );
}
...
export function StickerBook({
  ...
  stickers: Array<{ id: string; label: string; earned: boolean }>;
}) {
  return (
    <div
    ...
    >
      {stickers.map((s) => (
        <Card key={s.id} style={{ padding: 12, opacity: s.earned ? 1 : 0.5 }}>
          {s.label}
        </Card>
      ))}
    </div>
...
export function AudioControlBar({ volume = 60, speed = 1 }: { volume?: number; speed?: number }) {
  return (
    <Card style={{ padding: 16, display: "flex", gap: 12, alignItems: "center" }}>
      <span>🔊 {volume}%</span>
      <span>⏱ {speed}x</span>
      <Button variant="audio">Voice</Button>
    </Card>
  );
}
...
export function AgeModeSwitcher({
  ...
  mode: "sprout" | "spark" | "scholar";
  onModeChange: (mode: "sprout" | "spark" | "scholar") => void;
}) {
  return (
    <div role="group" aria-label="Age mode">
      {(["sprout", "spark", "scholar"] as const).map((m) => (
        <Button key={m} variant={mode === m ? "primary" : "ghost"} onClick={() => onModeChange(m)}>
          {m}
        </Button>
      ))}
    </div>
