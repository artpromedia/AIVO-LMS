...
| **`splash-icon.png`** | **Kept (regeneration recommended)** | The asset is 1024×1024 — a correct density for Expo — but is RGB/opaque rather than the transparent, centered lockup the spec calls for. No image tooling (ImageMagick / PIL) is available in this environment to regenerate it from `aivo-logo-*.png`. The new animated `SplashGate` (white wordmark on `#7C3AED`, matching `app.json` `backgroundColor`) provides the polished bridge and hands off from the native splash immediately, so any imperfection in the static asset is not user-visible for long. Regenerate at `imageWidth: 200`, transparent, centered when tooling is available. |
