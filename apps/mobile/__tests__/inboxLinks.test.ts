...
describe("mapInboxUrlToMobileRoute", () => {
  ...
  it("maps brain-review alias to brain", () => {
    expect(mapInboxUrlToMobileRoute("/dashboard/parent/learner/x/brain-review")).toEqual({
      pathname: "/(parent)/brain/[childId]",
      params: { childId: "x" },
    });
  });
  ...
  it("strips query and hash", () => {
    expect(mapInboxUrlToMobileRoute("/dashboard/parent/learner/x/iep?utm=email#goals")).toEqual({
      pathname: "/(parent)/iep/[childId]",
      params: { childId: "x" },
    });
  });
