...
export default function CaregiverGradebook() {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.title, { fontSize: type.h1.fontSize, lineHeight: type.h1.lineHeight }]}>{t("caregiverGradebook.title")}</Text>
      <Text style={styles.subtitle}>{t("caregiverGradebook.subtitle")}</Text>

      {domains.map((d) => (
        <AivoCard key={d.domain} style={styles.card}>
          <View style={styles.row}>
            <Text style={styles.name}>{d.domain}</Text>
            <Text style={styles.pct}>{d.masteryPercent}%</Text>
          </View>
          <View style={styles.bar}>
            <View style={[styles.fill, { width: `${d.masteryPercent}%` }]} />
          </View>
        </AivoCard>
      ))}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 15, fontFamily: "Nunito-Bold", color: colors.text },
  pct: { fontSize: 16, fontFamily: "Nunito-ExtraBold", color: colors.primary },
