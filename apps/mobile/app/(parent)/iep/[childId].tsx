      ...
      <AivoCard key={`${item.type}-${item.id}`} style={styles.timelineCard}>
        ...
        {item.type === "report" && (
          <>
            ...
            {p.narrative ? (
              <Text style={styles.timelineBody} numberOfLines={6}>
                {p.narrative}
              </Text>
            ) : null}
        ...
        {item.type === "amendment" && (
          <>
            ...
            {p.proposedByName ? (
              <Text style={styles.timelineMeta}>
                {t("parentIEP.proposedBy", { name: p.proposedByName })}
              </Text>
            ) : null}
            <View
              style={[
                styles.amendmentBadge,
                {
                  backgroundColor:
                    p.status === "merged" || p.status === "acknowledged"
                      ? colors.success + "20"
                      : p.status === "objected"
                        ? colors.error + "20"
                        : colors.accent + "20",
            ...
            >
              ...
              >
                {t(`parentIEP.amendStatus_${p.status}`) || p.status}
              </Text>
            </View>
            {p.status === "proposed" && (
              <View style={styles.amendmentActions}>
                ...
                <AivoButton
                  ...
                  onPress={() => {
                    setRespondModal({ id: p.id || item.id, summary: p.summary, mode: "objected" });
                    setRespondNote("");
                  }}
                  variant="outline"
                  size="sm"
            ...
            goals.map((goal: any) => (
              <AivoCard key={goal.id} style={styles.goalCard}>
                <View style={styles.goalHeader}>
                  ...
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          goal.status === "met" ? colors.success + "20" : colors.info + "20",
                      },
                    ]}
                  >
                    <Text
                ...
                <Text style={styles.progressText}>
                  {goal.progress}% {t("parentIEP.complete")}
                </Text>
              </AivoCard>
            ))
          )}
        ...
        <Pressable accessibilityRole="button" style={styles.modalOverlay} onPress={() => setPrefsModalOpen(false)}>
          ...
          >
            ...
            {!draftPrefs ? (
              ...
              (["progress_notes", "reports", "amendments", "reminders"] as IEPPrefCategory[]).map(
                (cat) => {
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: spacing.md }}>
              <AivoButton
                ...
                onPress={() => setPrefsModalOpen(false)}
                variant="outline"
                size="sm"
                style={{ flex: 1 }}
              />
          ...
          >
            ...
            <View style={{ flexDirection: "row", gap: 8, marginTop: spacing.md }}>
              ...
              <AivoButton
                title={
                  respondModal?.mode === "acknowledged"
                    ? t("parentIEP.confirmAcknowledge")
                    : t("parentIEP.confirmObject")
                }
