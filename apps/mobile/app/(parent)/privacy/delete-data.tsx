...
export default function ParentDeleteDataScreen() {
  ...
  const confirm = () => {
    Alert.alert(
      ...
      t("parentPrivacy.deleteConfirmBody", "We'll begin a reviewed deletion of your account data. This can't be undone once it completes."),
      [
        { text: t("common.cancel", "Cancel"), style: "cancel" },
        { text: t("parentPrivacy.deleteConfirmCta", "Request deletion"), style: "destructive", onPress: submit },
      ],
    );
  };
  ...
  const submit = async () => {
    setBusy(true);
    try {
      const res = await apiFetch(API.IDENTITY, "/api/users/me/deletion-request", { method: "POST", body: JSON.stringify({}) });
      Alert.alert(
        res.ok ? t("parentPrivacy.deleteSubmitted", "Request submitted") : t("common.error", "Something went wrong"),
        res.ok
          ? t("parentPrivacy.deleteSubmittedBody", "We've logged your deletion request and will confirm by email.")
          : t("parentPrivacy.deleteFailed", "Couldn't submit the request. Please contact support."),
      );
    } catch {
      Alert.alert(t("common.error", "Something went wrong"), t("parentPrivacy.deleteFailed", "Couldn't submit the request. Please contact support."));
    } finally {
      setBusy(false);
    }
  };
    ...
    <ResponsiveScreen maxWidth="reading" background={palette.bgPage}>
      ...
      <Card tone="raised" style={{ gap: spacing.sm }}>
        ...
        <Text style={[styles.body, { color: palette.inkMuted }]}>
          {t("parentPrivacy.deleteBody", "You can request deletion of your account and your children's data. We process requests in line with COPPA, FERPA, and applicable privacy laws.")}
        </Text>
        <Button
...
const styles = StyleSheet.create({
  warn: { flexDirection: "row", alignItems: "center", gap: 8, padding: spacing.md, borderRadius: radius.lg, borderWidth: 1 },
