...
export default function ParentLearnerSettingsScreen() {
  ...
  const LINKS: { icon: keyof typeof Ionicons.glyphMap; label: string; route: string }[] = [
    { icon: "person", label: t("learnerSettings.profile", "Profile"), route: `/(parent)/learners/${id}` },
    { icon: "accessibility", label: t("a11y.title", "Accessibility"), route: `/(parent)/accessibility/${id}` },
    { icon: "color-palette", label: t("parentSensory.title", "Sensory profile"), route: `/(parent)/sensory/${id}` },
    { icon: "clipboard", label: t("parentHub.iep", "IEP / Supports"), route: `/(parent)/iep/${id}` },
  ];
        ...
        <View style={{ gap: spacing.md }}>
          <Card tone="raised" style={{ gap: 4 }}>
            ...
            <Text style={[styles.meta, { color: palette.inkMuted }]}>
              {t("common.grade", { grade: learner?.gradeLevel, defaultValue: `Grade ${learner?.gradeLevel ?? ""}` })}
            </Text>
          </Card>
          ...
          <Card tone="raised" style={{ gap: 0 }}>
            {LINKS.map((l, i) => (
              <Pressable
                ...
                style={[styles.linkRow, i > 0 && { borderTopWidth: 1, borderTopColor: palette.border }]}
              >
          ...
          <Text style={[styles.note, { color: palette.inkMuted }]}>
            {t("learnerSettings.deleteNote", "Removing a learner is handled from account settings for safety.")}
          </Text>
        </View>
      )}
    </ResponsiveScreen>
...
const styles = StyleSheet.create({
  ...
  deleteBtn: {
    ...
    paddingVertical: 14, borderRadius: radius.xl, borderWidth: 1.5,
  },
