 ...
 * (mobile).
 *
 * This is the mobile source of truth for the teacher assessment wizard: the
 * section order, every question, its input type, and (for choice questions)
 * the allowed options. It is a faithful port of web-v2's
 * `lib/teacher/assessment-content.ts` so the two surfaces stay in sync — the
 * mobile wizard renders from it and the submit projection reads option LABELS
 * from it so the assessment-svc payload (system of record) stays
 * human-readable. (Like the parent assessment, content is duplicated per app
 * rather than shared; there is no cross-app assessment-content package.)
 *
 * Copy uses the `{name}` token for the learner's first name; the UI replaces it
 * at render time via `withName`. The projection never needs the name, so it
 * reads values/labels directly.
 *
 * 54 questions total:
...
export const TEACHER_ASSESSMENT_TOTAL_REQUIRED = TEACHER_ASSESSMENT_SECTION_ORDER.reduce(
  (n, id) => n + requiredQuestionCount(id),
  0,
);
