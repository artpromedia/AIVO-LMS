/**
 * Calm Corner — pure activity catalog + selectors (mobile port).
 *
 * A faithful, framework-free port of the web catalog
 * (apps/web-v2/lib/learner/calm.ts). No I/O, no React, no native imports:
 * every export is a deterministic, total function so the screen stays thin
 * and the unit tests are exhaustive. Activity ids mirror the web union (and
 * the homework-svc SelfRegulationPrompt union) so a homework focus signal
 * can deep-link straight into the right activity on both platforms.
 *
 * i18n keys resolve under the mobile `learnerCalm.*` namespace.
 */
...
export interface CalmActivity {
   ...
   *  reduce-motion (only the breathing orb today). */
  motionSafe: boolean;
  /** i18n key suffixes resolved under the `learnerCalm.activities` namespace. */
  titleKey: string;
  bodyKey: string;
}
 ...
 *  `learnerCalm.affirmations`. Total over the activity union. */
