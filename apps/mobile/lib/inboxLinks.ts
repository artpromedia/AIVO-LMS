 ...
 */
export type MobileRoute = { pathname: string; params?: Record<string, string> } | null;
...
export function mapInboxUrlToMobileRoute(input: string | null | undefined): MobileRoute {
  ...
  if (
    path === "/dashboard/parent/recommendations" ||
    path.startsWith("/dashboard/parent/recommendations/")
  ) {
    return { pathname: "/(parent)/recommendations" };
  }
  if (path === "/dashboard/parent/inbox" || path.startsWith("/dashboard/parent/inbox/")) {
    return { pathname: "/(parent)/inbox" };
  }
  if (path === "/dashboard/parent/billing" || path.startsWith("/dashboard/parent/billing/")) {
