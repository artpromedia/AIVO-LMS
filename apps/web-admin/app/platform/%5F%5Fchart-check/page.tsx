...
import { AdminCard, AdminKpiCard, AdminPageFrame, ChartCard, bucketByDay } from "@aivo/admin-ui";

// This is the one route that ships every chart primitive at once, so the
// heavy recharts surfaces load as split chunks (still SSR'd).
const AreaTrend = dynamic(() => import("@aivo/admin-ui").then((m) => m.AreaTrend));
const BarSeries = dynamic(() => import("@aivo/admin-ui").then((m) => m.BarSeries));
const DonutBreakdown = dynamic(() => import("@aivo/admin-ui").then((m) => m.DonutBreakdown));
const Funnel = dynamic(() => import("@aivo/admin-ui").then((m) => m.Funnel));
const Gauge = dynamic(() => import("@aivo/admin-ui").then((m) => m.Gauge));
const Sparkline = dynamic(() => import("@aivo/admin-ui").then((m) => m.Sparkline));
    ...
    >
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <ChartCard
          ...
          srRows={bucketByDay(SIGNUPS, (signup) => signup.createdAt).map((point) => ({
            day: point.t,
            signups: point.value,
          }))}
        >
        ...
        <ChartCard
          ...
          aspect={16 / 10}
          srRows={DONUT_SLICES.map((slice) => ({ kind: slice.label, tenants: slice.value }))}
        >
          <DonutBreakdown
            data={DONUT_SLICES}
        ...
        <ChartCard
          ...
          srRows={BAR_DATA.map((bar) => ({ subject: bar.label, lessons: bar.value }))}
        >
          <BarSeries
            data={BAR_DATA}
        ...
        <ChartCard
          ...
          aspect={16 / 10}
          srRows={[{ metric: "completion", percent: Math.round(GAUGE_RATIO * 100) }]}
        >
          <Gauge
            ratio={GAUGE_RATIO}
        ...
        <ChartCard
          ...
          srRows={FUNNEL_STAGES.map((stage) => ({ stage: stage.label, value: stage.value }))}
        >
          <Funnel
