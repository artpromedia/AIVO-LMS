...
export default async function NotFound() {
  ...
  const recoveryHref = session ? ROLE_HOME[session.role] : "/login";
  const recoveryLabel = session ? "Return to your admin home" : "Return to sign in";
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-6">
      <section className="admin-card max-w-lg p-8 text-center">
        ...
        <Link className="admin-button mt-6" href={recoveryHref}>
          {recoveryLabel}
        </Link>
      </section>
    </main>
