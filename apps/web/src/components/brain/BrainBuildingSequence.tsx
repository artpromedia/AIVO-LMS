  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-heading font-bold text-lg">
                Building {learnerName}&apos;s Brain
              </h1>
              <p className="text-white/40 text-xs font-body">
                Real-time construction from assessment data
              </p>
            </div>
          </div>
        ...
        <div
          className={`transition-all duration-400 ${stageVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          ...
          {currentStage === "activation" && (
            <StageActivation learnerName={learnerName} activationPulse={activationPulse} />
          )}
          ...
          {currentStage === "complete" && (
            <StageComplete learnerName={learnerName} onContinue={onSequenceComplete} />
          )}
        </div>
    ...
    <div className="text-center py-8">
      ...
      >
        <div className="relative mx-auto w-48 h-48 mb-6">
          <div
            className="absolute inset-0 rounded-full border-2 border-dashed border-purple-400/40 animate-spin"
            style={{ animationDuration: "20s" }}
          />
          <div
            className="absolute inset-4 rounded-full border border-dashed border-cyan-400/30 animate-spin"
        ...
        <p className="text-white/50 font-body text-sm max-w-md mx-auto">
          Selecting a baseline Brain template for {learnerName}&apos;s grade level and learning
          profile.
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        {(() => {
          const I = subjectIcon(domain);
          return <I className="w-5 h-5" style={{ color }} strokeWidth={2.5} aria-hidden />;
        })()}
      ...
      <div className="flex items-center justify-between text-xs">
        ...
        {gap > 0 && <span className="text-amber-400/80 font-body font-semibold">Gap: {gap}yr</span>}
      </div>
    </div>
  );
}

function StageDomains({
  learnerName,
  enrolled,
  masteryDecisions,
  domainsRevealed,
    ...
    <div>
      <div className="text-center mb-6">
        ...
        <p className="text-white/40 text-sm font-body">
          Loading domain scores from the Baseline Assessment
        </p>
      </div>
      ...
      {domainsRevealed >= masteryDecisions.length && (
        <div className="mt-4 bg-white/5 rounded-xl p-4 border border-white/10 animate-fadeIn">
          <p className="text-white/60 text-sm font-body text-center">
            {learnerName} is enrolled in Grade {enrolled}. Their Brain will deliver content at their
            actual functioning level per subject, not at Grade {enrolled} across the board. As
            mastery improves, the delivery level rises automatically.
          </p>
        </div>
      )}
    </div>
...
function StageAccommodations({
  learnerName,
  accommodationDecisions,
  accommodationsRevealed,
}: {
  learnerName: string;
  accommodationDecisions: AccommodationDecision[];
  accommodationsRevealed: number;
}) {
  return (
    <div>
      <div className="text-center mb-6">
        ...
        <p className="text-white/40 text-sm font-body">
          Configuring personalized supports based on evidence
        </p>
      </div>
      ...
      {accommodationDecisions.length === 0 ? (
        <div className="text-center py-8">
          <Star
            className="w-12 h-12 mx-auto mb-3 text-amber-400 fill-amber-300"
            strokeWidth={2}
            aria-hidden
          />
          <p className="text-white/50 font-body">
            No additional accommodations needed at this level.
          </p>
        </div>
      ) : (
        <div className="grid gap-3">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-white/40 text-xs font-body leading-relaxed">{a.reasoning}</p>
                </div>
                <span
                  className={`text-[10px] font-bold px-2 py-1 rounded-full whitespace-nowrap ${
                    a.source === "iep"
                      ? "bg-amber-500/20 text-amber-300"
                      : a.source === "parent_assessment"
      ...
      {accommodationsRevealed >= accommodationDecisions.length &&
        accommodationDecisions.length > 0 && (
          <div className="mt-4 bg-white/5 rounded-xl p-4 border border-white/10">
            <p className="text-white/60 text-sm font-body text-center">
              These accommodations will be applied to every lesson, tutor session, and activity
              automatically. You can adjust these at any time from your parent dashboard.
            </p>
          </div>
        )}
    </div>
...
function StageGoals({
  ...
  return (
    <div>
      <div className="text-center mb-6">
        <h2 className="font-heading font-bold text-xl text-white mb-1">
          Goal Mapping & Learning Paths
        </h2>
        <p className="text-white/40 text-sm font-body">
          Plotting the step-by-step journey for each domain
        </p>
      </div>
      ...
      {domainsWithGaps.length === 0 ? (
        <div className="text-center py-8">
          ...
          />
          <p className="text-white/50 font-body">
            All domains are performing at or near grade level!
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  {(() => {
                    const I = subjectIcon(m.domain);
                    return (
                      <I className="w-4 h-4" style={{ color }} strokeWidth={2.5} aria-hidden />
                    );
                  })()}
      ...
      {goalsRevealed >= domainsWithGaps.length && domainsWithGaps.length > 0 && (
        <div className="mt-4 bg-white/5 rounded-xl p-4 border border-white/10">
          <p className="text-white/60 text-sm font-body text-center">
            {learnerName}&apos;s Brain has planned a step-by-step path for every subject. Content
            will be delivered at their actual level, targeting their enrolled grade objectives.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="text-center py-8">
      <div className="relative mx-auto w-48 h-48 mb-6">
        <div
          className={`absolute inset-0 rounded-full transition-all duration-1000 ${
            activationPulse
              ? "bg-gradient-to-br from-purple-500/30 to-cyan-500/30 scale-110"
        ...
        <div
          className={`absolute inset-4 rounded-full transition-all duration-700 ${
            activationPulse
              ? "bg-gradient-to-br from-purple-500/40 to-emerald-500/40 scale-105"
              : "bg-white/5 scale-100"
          }`}
        />
        <div
          className={`absolute inset-8 rounded-full transition-all duration-500 ${
            activationPulse
              ? "bg-gradient-to-br from-cyan-500/50 to-purple-500/50 scale-100"
              : "bg-white/5 scale-100"
          }`}
        />
        <div className="absolute inset-0 flex items-center justify-center">
          <Brain
            className={`text-white transition-all duration-500 ${activationPulse ? "scale-110" : "scale-100"}`}
      ...
      <div className="mt-6 bg-white/5 rounded-xl p-4 border border-white/10">
        <p className="text-white/60 text-sm font-body text-center">
          {learnerName}&apos;s Brain is now active. It will update after every learning session. All
          changes are versioned. You can roll back to any previous state at any time. Your
          child&apos;s Brain data is encrypted and you can export or delete it at any time.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div className="text-center mb-6">
        ...
        <p className="text-white/40 text-sm font-body">
          Calibrating each AI tutor to {learnerName}&apos;s levels
        </p>
      </div>
      ...
      <div className="grid gap-3">
        {tutorDecisions.map((t, i) => {
          ...
          return (
            ...
            >
              <div
                className="w-12 h-12 rounded-full overflow-hidden border-2 flex-shrink-0 shadow-lg"
      ...
      {tutorsRevealed >= tutorDecisions.length && (
        <div className="mt-4 bg-white/5 rounded-xl p-4 border border-white/10">
          <p className="text-white/60 text-sm font-body text-center">
            Every tutor reads {learnerName}&apos;s Brain before each session. They know the exact
            level to teach at, which accommodations to apply, and which skills to target next.
          </p>
        </div>
      )}
    </div>
...
function StageComplete({
  ...
  return (
    <div
      className={`text-center py-8 transition-all duration-700 ${visible ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
    >
      ...
      <p className="text-white/50 text-sm font-body mb-8 max-w-md mx-auto">
        The Brain has been built from {learnerName}&apos;s assessment data. Review the details below
