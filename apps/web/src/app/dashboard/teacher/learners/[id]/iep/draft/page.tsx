...
export default function IepDraftEditorPage() {
  ...
  const [newMember, setNewMember] = useState<{ userId: string; role: (typeof TEAM_ROLES)[number] }>(
    { userId: "", role: "gen_ed_teacher" },
  );
  ...
  const schedulePlopSave = useCallback(
    (area: string, narrative: string) => {
      if (!activeId) return;
      const draftId = activeId;
      const buf = pendingByDraft.current[draftId] || {};
  ...
  const scheduleGoalSave = useCallback(
    (goalId: string, patch: Record<string, any>) => {
      if (!activeId) return;
      const draftId = activeId;
      const buf = pendingByDraft.current[draftId] || {};
      buf.goals = buf.goals || {};
  ...
  const scheduleServiceSave = useCallback(
    (sId: string, patch: Record<string, any>) => {
      if (!activeId) return;
      const draftId = activeId;
      const buf = pendingByDraft.current[draftId] || {};
  ...
  const savePresentLevel = useCallback(
    async (area: string, narrative: string) => {
  ...
  const updateGoal = (goalId: string, patch: Partial<Goal>) => {
    ...
    setBundle({
      ...bundle,
      goals: bundle.goals.map((g) => (g.id === goalId ? { ...g, ...patch } : g)),
    });
  };
  ...
  const deleteGoal = async (goalId: string) => {
    ...
    if (r.ok && bundle)
      setBundle({ ...bundle, goals: bundle.goals.filter((g) => g.id !== goalId) });
  };
  ...
  const updateService = (sId: string, patch: Partial<Service>) => {
    ...
    setBundle({
      ...bundle,
      services: bundle.services.map((s) => (s.id === sId ? { ...s, ...patch } : s)),
    });
  };
  ...
  const deleteService = async (sId: string) => {
    ...
    if (r.ok && bundle)
      setBundle({ ...bundle, services: bundle.services.filter((s) => s.id !== sId) });
  };
  ...
  const addTeamMember = async () => {
    ...
    if (r.ok) {
      setNewMember({ userId: "", role: newMember.role });
      refreshCollab(activeId);
    }
  };
  ...
  const missing = useMemo(() => {
    ...
    const plopHas = (PLOP_AREAS as readonly string[]).every(
      (a) => (bundle.presentLevels.find((p) => p.area === a)?.narrative || "").trim().length > 0,
    );
      ...
      <header className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {bundle && (
            ...
            >
              {t(`status_${bundle.profile.lifecycleState}`)}
            </span>
          )}
          <button
            onClick={createDraft}
      ...
      {drafts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6">
          <aside className="vi-card p-3 space-y-1 self-start">
            <p className="text-xs vi-text-muted font-bold uppercase tracking-wide px-2 py-1">
              {t("drafts")}
            </p>
            {drafts.map((d) => (
              <button
              ...
              >
                ...
                <div className="text-[10px] uppercase tracking-wide vi-text-muted">
                  {t(`status_${d.lifecycleState}`)}
                </div>
              </button>
            ))}
            ...
            {SECTIONS.map((s) => (
              <div key={s} className="flex items-center gap-2 px-2 py-1 text-xs">
                {missing.has(s) ? (
                  <AlertCircle
                    size={14}
                    className="text-[hsl(var(--visual-sel))]"
                    aria-hidden="true"
                  />
                ) : (
                  <CheckCircle2
            ...
            <section key={bundle.profile.id} className="space-y-4">
                  ...
                  {PLOP_AREAS.map((area) => {
                    ...
                    return (
                      <div key={`${bundle.profile.id}-${area}`} className="vi-card p-4 space-y-2">
                        <label className="block text-sm font-bold vi-text">
                          {t(`plop_${area}`)}
                        </label>
                        <textarea
                  ...
                  {bundle.goals.map((goal) => (
                    <div key={goal.id} className="vi-card p-4 space-y-2">
                      <textarea
                        ...
                        disabled={!editable}
                        onChange={(e) => {
                          updateGoal(goal.id, { goalText: e.target.value });
                          scheduleGoalSave(goal.id, { goalText: e.target.value });
                        }}
                        onBlur={(e) => persistGoal(goal.id, { goalText: e.target.value })}
                        rows={2}
                        placeholder={t("goal_text_placeholder")}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <input
                          ...
                          disabled={!editable}
                          onChange={(e) => {
                            updateGoal(goal.id, { domain: e.target.value });
                            scheduleGoalSave(goal.id, { domain: e.target.value });
                          }}
                        ...
                        <input
                          ...
                          disabled={!editable}
                          onChange={(e) => {
                            updateGoal(goal.id, { baseline: e.target.value });
                            scheduleGoalSave(goal.id, { baseline: e.target.value });
                          }}
                        ...
                        <input
                          ...
                          disabled={!editable}
                          onChange={(e) => {
                            updateGoal(goal.id, { targetCriteria: e.target.value });
                            scheduleGoalSave(goal.id, { targetCriteria: e.target.value });
                          }}
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => aiDraftGoal(goal)}
                          disabled={!editable || drafting === goal.id}
                          style={{ minHeight: 44 }}
              ...
              {section === "accommodations" && (
                <div className="vi-card p-4 space-y-2">
                  <label className="block text-sm font-bold vi-text">
                    {t("accommodations_label")}
                  </label>
                  <textarea
                    key={`acc-${bundle.profile.id}`}
                    defaultValue={(bundle.profile.accommodations || [])
                      .map((a: any) =>
                  ...
                  {bundle.services.map((s) => (
                    <div key={s.id} className="vi-card p-4 grid grid-cols-1 md:grid-cols-6 gap-2">
                      <input
                        ...
                        disabled={!editable}
                        onChange={(e) => {
                          updateService(s.id, { serviceType: e.target.value });
                          scheduleServiceSave(s.id, { serviceType: e.target.value });
                        }}
                      ...
                      />
                      <input
                        value={s.providerRole || ""}
                        disabled={!editable}
                        onChange={(e) => {
                          updateService(s.id, { providerRole: e.target.value });
                          scheduleServiceSave(s.id, { providerRole: e.target.value });
                        }}
                      ...
                      <input
                        type="number"
                        value={s.minutesPerWeek ?? ""}
                        disabled={!editable}
                        onChange={(e) => {
                          const v = e.target.value ? Number(e.target.value) : null;
                          updateService(s.id, { minutesPerWeek: v });
                          scheduleServiceSave(s.id, { minutesPerWeek: v });
                        }}
                        onBlur={(e) =>
                          persistService(s.id, {
                            minutesPerWeek: e.target.value ? Number(e.target.value) : null,
                          })
                        }
                      ...
                      <input
                        ...
                        disabled={!editable}
                        onChange={(e) => {
                          updateService(s.id, { frequency: e.target.value });
                          scheduleServiceSave(s.id, { frequency: e.target.value });
                        }}
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          disabled={!editable}
                          onChange={(e) => {
                            updateService(s.id, { location: e.target.value });
                            scheduleServiceSave(s.id, { location: e.target.value });
                          }}
              ...
              {section === "placement" && (
                <div className="vi-card p-4 space-y-2">
                  ...
                  <input
                    value={bundle.profile.placement || ""}
                    disabled={!editable}
              ...
              {section === "review" && (
                <div className="vi-card p-4 space-y-2">
                  ...
                  <input
                    ...
                    disabled={!editable}
                    value={
                      bundle.profile.reviewDate
                        ? new Date(bundle.profile.reviewDate).toISOString().split("T")[0]
                        : ""
                    }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[1fr_180px_auto] gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewMember({
                            ...newMember,
                            role: e.target.value as (typeof TEAM_ROLES)[number],
                          })
                        }
                        className="vi-input rounded-lg px-3 py-2 text-sm"
                      >
                      ...
                      team.map((m) => (
                        <div key={m.id} className="flex items-center gap-3 p-3">
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-bold vi-text truncate">
                    ...
                    {sigStatus && (
                      <>
                        <p className="text-xs vi-text-muted">
                          {tc("required_roles")}:{" "}
                          {sigStatus.required.map((r) => tc(`role_${r}`)).join(", ")}
                        </p>
                        {sigStatus.missingRoles.length > 0 ? (
                          <p className="text-xs font-bold text-[hsl(var(--visual-sel))]">
                            {tc("awaiting")}:{" "}
                            {sigStatus.missingRoles.map((r) => tc(`role_${r}`)).join(", ")}
                          </p>
                        ) : (
              ...
              {(
                ["plop", "goals", "accommodations", "services", "placement", "review"] as Section[]
              ).includes(section) && (
                <div className="vi-card p-4 space-y-3">
                  <div className="flex items-center gap-2 vi-text font-bold text-sm">
                  ...
                  <div className="space-y-2 max-h-72 overflow-auto">
                    {sectionComments.length === 0 ? (
                      ...
                      sectionComments.map((c) => (
                        <div
                          key={c.id}
                          className={`p-3 rounded-lg border vi-border ${c.resolvedAt ? "opacity-60" : ""}`}
                        >
                          <div className="flex items-center gap-2 text-xs vi-text-muted">
                            <span className="font-bold vi-text">
...
function UpdatesSection({
  ...
  const refreshAll = useCallback(async () => {
    ...
    const [n, r, a] = await Promise.all([
      ...
      fetch(`/api/iep/drafts/${draftId}/amendments`, { headers }).then((x) =>
        x.ok ? x.json() : [],
      ),
    ]);
  ...
  const createAmendment = async () => {
    ...
    if (amendChanges.trim()) {
      try {
        parsed = JSON.parse(amendChanges);
      } catch {
        parsed = { notes: amendChanges };
      }
    }
    try {
      ...
      <div className="vi-card p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_auto] gap-2">
          <textarea
            ...
            placeholder={tu("note_placeholder")}
            rows={2}
            className="vi-input rounded-lg px-3 py-2 text-sm"
          />
          <select
          ...
          >
            ...
            {goals.map((g) => (
              <option key={g.id} value={g.id}>
                {g.domain || ""}: {g.goalText.slice(0, 40)}
              </option>
            ))}
          </select>
          <select
        ...
        <div className="vi-card p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-2">
            ...
            <textarea
              ...
              placeholder={tu("narrative_placeholder")}
              rows={4}
              className="vi-input rounded-lg px-3 py-2 text-sm"
            />
          </div>
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              disabled={creatingReport || !reportPeriod.trim()}
              style={{ minHeight: 40 }}
              ...
              reports.map((r) => (
                <div key={r.id} className="p-3 rounded-lg border vi-border">
                  <div className="flex items-center gap-2 text-xs vi-text-muted">
                    ...
                    <span
                      className={`ml-auto px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${
                        r.status === "sent"
                  ...
                  {editingReportId === r.id ? (
                    ...
                    r.narrative && (
                      <p className="text-sm vi-text mt-1 whitespace-pre-wrap line-clamp-4">
                        {r.narrative}
                      </p>
                    )
                  )}
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const resp = await fetch(
                                  `/api/iep/drafts/${draftId}/reports/${r.id}`,
                                  {
                                    method: "PATCH",
                                    headers: { ...headers, "Content-Type": "application/json" },
                        ...
                        <>
                          <button
                            onClick={() => {
                              ...
                              setEditingReportText(r.narrative || "");
                            }}
                            style={{ minHeight: 36 }}
                            className="px-3 py-1.5 rounded-full vi-surface-soft text-xs font-bold"
                          >
      ...
      {isCaseManager && isFinalised && (
        <div className="vi-card p-4 space-y-3">
          ...
          <textarea
            ...
            placeholder={tu("amendment_changes_placeholder")}
            rows={4}
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={creatingAmend || !amendSummary.trim()}
              style={{ minHeight: 40 }}
