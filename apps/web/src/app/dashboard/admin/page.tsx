...
export default function AdminOverview() {
  ...
  const statCards: {
  ...
  }[] = [
    {
      ...
      Icon: User,
      bg: "bg-[hsl(var(--visual-primary))]",
      link: "/dashboard/admin/users",
    },
    {
      title: td("total_learners"),
      value: stats?.totalLearners ?? "—",
      Icon: GraduationCap,
      bg: "bg-[hsl(var(--visual-science))]",
      link: "/dashboard/admin/learners",
    },
    {
      title: t("tenants"),
      value: stats?.totalTenants ?? "—",
      Icon: Building2,
      bg: "bg-[hsl(var(--visual-math))]",
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm vi-text-muted mt-1">
              {td("welcome_message", { name: user?.name ?? "" })}
            </p>
          </div>
        </div>
        ...
        {statCards.map((c) => {
          ...
          return (
            <Link key={c.title} href={c.link} className="group">
              <div
                className={`${c.bg} rounded-2xl p-5 text-white shadow-lg group-hover:shadow-xl transition-all group-hover:scale-[1.02]`}
              >
        ...
        <div className="lg:col-span-2 vi-card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link
              ...
              className="text-xs text-[hsl(var(--visual-primary))] font-semibold hover:underline"
            >
              View All →
            </Link>
          </div>
                ...
                return roleCounts.map((rc) => {
                  ...
                  return (
                    <div key={rc.role} className="flex items-center gap-4">
                      <span
                        className={`px-3 py-1 text-xs rounded-full font-semibold w-36 text-center ${ROLE_COLORS[rc.role] || "vi-surface-soft vi-text-muted"}`}
                      >
                        {rc.role.replace(/_/g, " ")}
                      </span>
                      <div className="flex-1 vi-surface-soft rounded-full h-3 overflow-hidden">
                        <div
                          className="h-full bg-[hsl(var(--visual-primary))] rounded-full transition-all"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
            ...
            {quickActions.map((a) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-semibold vi-text-muted group-hover:text-[hsl(var(--visual-primary))]">
                    {a.label}
                  </span>
                </Link>
              );
          ...
          <div className="space-y-2">
            {(statusOverview?.services || []).map((svc: any) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  {svc.latencyMs > 0 && (
                    <span className="text-xs vi-text-muted">{svc.latencyMs}ms</span>
                  )}
                  <span
            ...
            <div className="space-y-2">
              {stats.recentUsers.slice(0, 8).map((u) => (
                ...
                >
                  ...
                  <span
                    className={`px-2 py-0.5 text-xs rounded-full font-medium ${ROLE_COLORS[u.role] || "vi-surface-soft"}`}
                  >
                    {u.role.replace(/_/g, " ")}
                  </span>
                </div>
