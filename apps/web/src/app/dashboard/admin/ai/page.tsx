    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm vi-text-muted mt-1">
            Monitor brain clones, AI model performance, and content generation across the platform.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="vi-card p-6">
          ...
          <p className="text-xs vi-text-muted mb-4">
            Priority: Claude 4.7 adaptive (Opus → Sonnet → Haiku) → Gemini 3.0 Pro → GPT-5.5
          </p>
          <div className="space-y-3">
            {AI_PROVIDERS.map((p) => (
              <div
                key={p.name}
                className="flex items-center justify-between p-4 rounded-xl border vi-border hover:border-[hsl(var(--visual-primary)/0.3)] transition"
              >
                <div className="flex items-center gap-3">
                  <Circle
                    size={20}
                    strokeWidth={2.5}
                    className={`${p.color} fill-current`}
                    aria-hidden="true"
                  />
                  <div>
                    <p className="text-sm font-semibold vi-text">{p.name}</p>
                    <p className="text-xs vi-text-muted">
                      {p.provider} &middot; {p.model}
                    </p>
                  </div>
                </div>
                <span
            ...
            {[
              {
                ...
                desc: "6-domain Discovery Adventure",
                status: "active",
              },
              {
                step: "2",
                label: "Parent Questionnaire",
                desc: "49-question intake form (11 categories)",
                status: "active",
              },
              {
                step: "3",
                label: "Functioning Level Classification",
                desc: "5-level auto-classification",
                status: "active",
              },
              {
                step: "4",
                label: "Brain State Creation",
                desc: "Mastery levels, accommodations, tutor config",
                status: "active",
              },
              {
                step: "5",
                label: "XAI Explanation Generation",
                desc: "Transparent decision rationale",
                status: "active",
              },
              {
                step: "6",
                label: "Parent Review & Approval",
                desc: "Approve / Add Context / Start Over",
                status: "active",
              },
            ].map((s) => (
      ...
      <div className="vi-card p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            {
              label: "COPPA Consent Gate",
              status: true,
              detail: "Parental consent required before data collection",
            },
            {
              label: "Content Safety Filter",
              status: true,
              detail: "All AI-generated content passes quality & safety checks",
            },
            {
              label: "Bias Detection",
              status: true,
              detail: "Content checked for cultural and demographic sensitivity",
            },
            {
              label: "Data Encryption",
              status: true,
              detail: "Brain state encrypted at rest (AES-256)",
            },
            {
              label: "Parent Approval Loop",
              status: true,
              detail: "Level changes and accommodations require parent consent",
            },
            {
              label: "Full Audit Trail",
              status: true,
              detail: "Every brain version tracked with decision rationale",
            },
          ].map((c) => (
            <div
              key={c.label}
              className="flex items-start gap-3 p-4 rounded-xl bg-[hsl(var(--visual-science)/0.08)] border border-[hsl(var(--visual-science)/0.25)]"
            >
              <Check
                size={16}
                strokeWidth={3}
      ...
      <div className="vi-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3 text-xs vi-text-muted">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[hsl(var(--visual-reading))]" /> 7 Core
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[hsl(var(--visual-sel))]" /> 7 Expansion
            </span>
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {(Object.entries(TUTORS) as [TutorKey, (typeof TUTORS)[TutorKey]][]).map(([key, t]) => (
            ...
            >
              <div
                className="relative w-12 h-12 rounded-full mx-auto mb-2 overflow-hidden border-2 group-hover:scale-110 transition-transform shadow-sm"
              ...
              <span
                className={`inline-block mt-1 px-1.5 py-0.5 text-[9px] font-bold rounded-full ${
                  t.tier === "core"
                    ? "bg-[hsl(var(--visual-reading)/0.08)] text-[hsl(var(--visual-reading))]"
                    : "bg-[hsl(var(--visual-sel)/0.10)] text-[hsl(var(--visual-sel))]"
                }`}
              >
                {t.tier}
              </span>
        ...
        {expandedTutor &&
          TUTORS[expandedTutor] &&
          (() => {
            const t = TUTORS[expandedTutor];
            return (
              <div
                className="mt-4 p-5 rounded-xl border-2 animate-in fade-in slide-in-from-top-2 duration-200"
                style={{ borderColor: t.color, backgroundColor: `${t.color}08` }}
              >
                <div className="flex items-start gap-4">
                  <div
                    className="relative w-20 h-20 rounded-xl overflow-hidden border-2 shadow-lg flex-shrink-0"
                    style={{ borderColor: t.color }}
                  >
                    <Image
                      src={t.avatar}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 bg-white rounded-lg p-3 border vi-border">
                      <p className="text-[10px] vi-text-muted font-semibold uppercase mb-1">
                        Functioning Level Adaptations
                      </p>
                      <div className="flex flex-wrap gap-1.5">
                        {["STANDARD", "SUPPORTED", "LOW_VERBAL", "NON_VERBAL", "PRE_SYMBOLIC"].map(
                          (level) => (
                            <span
                              key={level}
                              className={`px-2 py-0.5 text-[10px] rounded-full font-semibold ${LEVEL_COLORS[level] || "vi-surface-soft vi-text-muted"}`}
                            >
                              {level.replace(/_/g, " ")}
                            </span>
                          ),
