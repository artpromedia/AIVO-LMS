...
export default function AdminServicesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/status/uptime")
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
      fetch("/api/status/ops-alerts-outbox")
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
    ])
      .then(([overview, incidentData, uptimeData, outboxData]) => {
        if (overview?.services) {
          ...
          setOverallStatus(
            overview.overall === "major_outage" ? "outage" : overview.overall || "checking",
          );
        }
      ...
      fetch("/api/status/overview")
        ...
        .then((data) => {
          if (data?.services) {
            ...
            setOverallStatus(
              data.overall === "major_outage" ? "outage" : data.overall || "checking",
            );
          }
        })
        .catch(() => {});
  ...
  const downCount = serviceStatuses.filter(
    (s) => s.status !== "operational" && s.status !== "degraded",
  ).length;
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm vi-text-muted mt-1">
            Real-time health monitoring across all {ALL_SERVICES.length} microservices.
          </p>
        </div>
        <div
        ...
        >
          <span
            className={`w-2 h-2 rounded-full ${
              overallStatus === "operational"
                ? "bg-[hsl(var(--visual-science))]"
                : overallStatus === "degraded"
                  ? "bg-[hsl(var(--visual-sel))] animate-pulse"
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-[hsl(var(--visual-science)/0.08)] rounded-xl p-4 border border-[hsl(var(--visual-science)/0.25)] text-center">
          ...
          <p className="text-xs text-[hsl(var(--visual-science))] font-semibold mt-1">
            Operational
          </p>
        </div>
          ...
          <div className="divide-y divide-slate-50">
            {ALL_SERVICES.map((svc) => {
              ...
              return (
                <div
                  key={svc.name}
                  className="flex items-center justify-between px-5 py-4 hover:vi-bg/50 transition"
                >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {live?.latency !== undefined && live.latency > 0 && (
                      <span
                        className={`text-xs font-medium ${live.latency < 200 ? "text-[hsl(var(--visual-science))]" : live.latency < 500 ? "text-[hsl(var(--visual-sel))]" : "text-[hsl(var(--visual-math))]"}`}
                      >
                        {live.latency}ms
                      </span>
                    )}
      ...
      <div className="vi-card overflow-hidden">
        <div className="p-5 border-b vi-border">
          ...
          <p className="text-xs vi-text-muted mt-1">
            Per-service ops-alerts outbox depth. Warning state escalates to PagerDuty after 3
            consecutive checks.
          </p>
        </div>
        {outboxTiles.length === 0 ? (
          <div className="p-8 text-center vi-text-muted text-sm">
            No services reporting. Check OPS_ALERT_OUTBOX_SOURCES.
          </div>
        ) : (
              ...
              return (
                <div key={t.service} className={`rounded-xl border p-4 ${stateColor}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={`px-2 py-0.5 text-[10px] rounded-full font-semibold uppercase ${badgeColor}`}
                    >
                      {t.state}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-1 text-center">
                    ...
                    <div>
                      <p
                        className={`text-2xl font-bold ${t.droppedTotal > 0 ? "text-[hsl(var(--visual-math))]" : "vi-text"}`}
                      >
                        {t.droppedTotal}
                      </p>
                  ...
                  <div className="mt-3 pt-3 border-t border-current/10 text-[11px] vi-text-muted">
                    ...
                    {t.firstNonZeroAt && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium vi-text">
                          {new Date(t.firstNonZeroAt).toLocaleTimeString()}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      ...
                      {ls ? (
                        <span
                          className={`font-medium ${t.shutdownStale ? "vi-text-muted italic" : "vi-text"}`}
                        >
                    ...
                    <div className="flex justify-between">
                      ...
                      {t.lastAutoFlushAt ? (
                        <span
                          className={`font-medium ${t.autoFlushErrorsTotal > 0 ? "text-[hsl(var(--visual-sel))]" : "vi-text"}`}
                        >
                    ...
                    {ls && (
                      <div className="flex justify-between">
                        <span>at</span>
                        <span
                          className={`font-medium ${t.shutdownStale ? "italic text-[hsl(var(--visual-math))]" : "vi-text"}`}
                        >
      ...
      <div className="vi-card overflow-hidden">
        ...
        {showIncidentForm && (
          <form
            ...
            className="p-5 border-b vi-border flex gap-3 items-end vi-bg"
          >
            <div className="flex-1">
              <label
                htmlFor="incident-title"
                className="block text-xs font-medium vi-text-muted mb-1"
              >
                Title
              </label>
              <input
                ...
                required
                className="w-full px-3 py-2 rounded-lg border vi-border text-sm"
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-lg bg-[hsl(var(--visual-math))] text-white text-sm font-semibold"
            >
              Create
            </button>
          </form>
