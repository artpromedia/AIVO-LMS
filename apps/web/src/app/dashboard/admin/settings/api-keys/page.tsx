...
export default function ApiKeysPage() {
  ...
  const toggleScope = (scope: string) => {
    setNewScopes(
      newScopes.includes(scope) ? newScopes.filter((s) => s !== scope) : [...newScopes, scope],
    );
  };
  ...
  const handleRotate = async (id: string) => {
    if (!confirm("Rotate this key? The old key will continue working for 24 hours during overlap."))
      return;
    try {
  ...
  const handleRevoke = async (id: string) => {
    if (!confirm("Revoke this key immediately? Any caller using it will start receiving 401."))
      return;
    try {
  ...
  const status = (k: ApiKey): { label: string; tone: string } => {
    ...
    if (k.expiresAt && new Date(k.expiresAt) < new Date())
      return { label: "Expired", tone: "text-slate-500 bg-slate-100" };
    if (
      k.gracePeriodEndsAt &&
      new Date(k.gracePeriodEndsAt) > new Date() &&
      k.rotatedFromId === null
    ) {
    ...
    <div className="p-8 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm vi-text-muted mt-1">
              Manage API keys for programmatic access. Keys are hashed with argon2id and shown only
              once.
            </p>
          </div>
        </div>
      ...
      {createdKey && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5">
          ...
          <div className="bg-white border border-green-300 rounded-xl px-4 py-3 font-mono text-sm select-all break-all">
            {createdKey.plain}
          </div>
          <button
      ...
      {loading ? (
        ...
        <div className="vi-card p-12 text-center">
          ...
          <p className="text-sm vi-text-muted mt-1">
            Generate an API key to start using the AIVO REST API.
          </p>
        </div>
      ) : (
      ...
      {showCreate && !createdKey && (
        <div className="vi-card p-6 space-y-4">
          ...
          <div>
            <label htmlFor="key-name" className="block text-sm font-medium vi-text mb-1">
              Key Name
            </label>
            <input
              id="key-name"
          ...
          <div>
            <label htmlFor="key-expiry" className="block text-sm font-medium vi-text mb-1">
              Expires in (days, max 365)
            </label>
            <input
              id="key-expiry"
              type="number"
              ...
              onChange={(e) =>
                setNewExpiry(
                  e.target.value === ""
                    ? ""
                    : Math.max(1, Math.min(365, parseInt(e.target.value, 10))),
                )
              }
              className="w-full px-4 py-2.5 rounded-xl border vi-border text-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none"
            />
