        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm vi-text-muted mt-1">
              Manage schools and campuses within your district.
            </p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {schools.map((s) => (
            <Link
              key={s.id}
              href={`/dashboard/district/schools/${s.id}`}
              className="vi-card p-6 hover:shadow-md hover:border-[hsl(var(--visual-primary)/0.3)] transition-all block"
            >
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-[hsl(var(--visual-primary)/0.12)] text-[hsl(var(--visual-primary))] rounded-xl flex items-center justify-center">
                  <SchoolIcon size={20} strokeWidth={2.5} aria-hidden="true" />
                </div>
                ...
                <span
                  className={`px-2 py-0.5 text-xs rounded-full font-semibold ${s.status === "active" ? "bg-[hsl(var(--visual-science)/0.14)] text-[hsl(var(--visual-science))]" : "vi-surface-soft vi-text-muted"}`}
                >
                  {s.status}
                </span>
              </div>
    ...
    try {
      const res = await fetch("/api/district/schools", {
        ...
        body: JSON.stringify({
          ...
          principalEmail: principalEmail.trim() || undefined,
          enrollmentCapacity: capacity ? parseInt(capacity) : undefined,
        }),
      });
      if (!res.ok) {
        const d = await res.json();
        throw new Error(d.error || "Failed to create school");
      }
      onCreated();
    } catch (err: any) {
