...
export default function SsoSettingsPage() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [cfgRes, tokRes] = await Promise.all([
        ...
        fetch("/api/admin-svc/scim-tokens", {
      ...
      if (cfgRes.ok) {
        ...
        setRoleMapRaw(
          Object.entries(cfg.config?.roleMap || {})
            .map(([k, v]) => `${k}=${v}`)
            .join("\n"),
        );
      }
  ...
  const issueToken = async () => {
    ...
    try {
      ...
      setMessage({
        kind: "err",
        text: err instanceof Error ? err.message : "Could not issue token",
      });
    }
  };

  const revokeToken = async (id: string) => {
    if (!confirm("Revoke this SCIM token? Your IdP will lose provisioning access immediately."))
      return;
  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl">
      <div className="flex items-center gap-3 text-sm text-slate-500">
        <Link href="/dashboard/district/settings" className="hover:text-violet-700 transition">
          Settings
        </Link>
        <span>/</span>
      ...
      <header className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Connect your identity provider so staff sign in with their work credentials.
          </p>
        </div>
      </header>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6 space-y-5">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1" htmlFor="sso-preset">
            IdP type
          </label>
          <select
            id="sso-preset"
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1" htmlFor="sso-domains">
            Email domains owned by your district
          </label>
          <input
            id="sso-domains"
            type="text"
          ...
          <p className="mt-1 text-xs text-slate-500">
            When users enter emails on these domains, they&apos;re sent to your IdP.
          </p>
        </div>
        ...
        <label className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-slate-900">
            Require SSO (block password login for these domains)
          </span>
        </label>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1" htmlFor="sso-entry">
              SSO entry point (IdP redirect URL)
            </label>
            <input
              id="sso-entry"
              type="url"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1" htmlFor="sso-logout">
              SLO URL (optional)
            </label>
            <input
              id="sso-logout"
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-1" htmlFor="sso-cert">
          ...
          <textarea
            ...
            placeholder={
              config.idpCertSet
                ? "(stored — paste a new cert to replace)"
                : "-----BEGIN CERTIFICATE-----\n..."
            }
        ...
        <details className="text-sm">
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-slate-600 mb-1" htmlFor="email-attr">
                Email attribute
              </label>
              <input
                id="email-attr"
            ...
            <div>
              <label className="block text-xs text-slate-600 mb-1" htmlFor="name-attr">
                Name attribute
              </label>
              <input
                id="name-attr"
                value={config.nameAttribute || ""}
                onChange={(e) => setConfig({ ...config, nameAttribute: e.target.value })}
                className="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs font-mono"
              />
            </div>
            <div>
              <label className="block text-xs text-slate-600 mb-1" htmlFor="role-attr">
                Role attribute
              </label>
              <input
                id="role-attr"
                value={config.roleAttribute || ""}
          ...
          <div className="mt-3">
            <label className="block text-xs text-slate-600 mb-1" htmlFor="role-map">
              Role map (one per line, idp_value=AIVO_ROLE)
            </label>
            <textarea
              id="role-map"
              rows={4}
              value={roleMapRaw}
              onChange={(e) => setRoleMapRaw(e.target.value)}
          ...
          <div className="mt-3">
            <label className="block text-xs text-slate-600 mb-1" htmlFor="default-role">
              Default role (when no mapping matches)
            </label>
            <select
              id="default-role"
              value={config.defaultRole || "TEACHER"}
        ...
        {tenantId && (
          <div className="mt-4 p-3 bg-slate-50 rounded-lg text-xs text-slate-600 space-y-1">
            <p>
              <strong>ACS URL:</strong> <code className="break-all">{acsUrl}</code>
            </p>
            <p>
              <strong>Entity ID:</strong> <code>aivo:sp:{tenantId}</code>
            </p>
          </div>
        )}
      </section>
      ...
      <section className="bg-white border border-slate-200 rounded-2xl p-6 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Use one of these tokens as the bearer secret in your IdP&apos;s SCIM configuration.
              </p>
            </div>
          </div>
        </div>
        ...
        {createdToken && (
          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200">
            ...
            <div className="bg-white border border-emerald-300 rounded-lg px-3 py-2 font-mono text-xs break-all select-all">
              {createdToken}
            </div>
            <button
