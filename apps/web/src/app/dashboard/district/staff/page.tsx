      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm vi-text-muted mt-1">
            Manage teachers, therapists, and caregivers across your district.
          </p>
        </div>
        <button
            ...
            <tbody>
              {staff.map((u) => (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`px-2.5 py-0.5 text-xs rounded-full font-semibold ${ROLE_COLORS[u.role] || "vi-surface-soft vi-text-muted"}`}
                    >
                      {u.role.replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-5 py-3 vi-text-muted text-xs">
                    {u.schoolAssignments?.length
                      ? u.schoolAssignments.map((a) => a.schoolName).join(", ")
                      : "—"}
                  </td>
                  <td className="px-5 py-3 vi-text-muted text-xs">
              ...
              {staff.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-5 py-10 text-center vi-text-muted">
                    No staff found
                  </td>
                </tr>
              )}
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="vi-text-muted hover:vi-text-muted text-xl">
            &times;
          </button>
        </div>
        {error && (
          <p className="text-sm text-[hsl(var(--visual-math))] bg-[hsl(var(--visual-math)/0.08)] rounded-lg px-3 py-2">
            {error}
          </p>
        )}
        {result ? (
          <div className="space-y-3">
            <div className="bg-[hsl(var(--visual-science)/0.08)] border border-[hsl(var(--visual-science)/0.25)] rounded-xl p-4">
              <p className="text-sm font-semibold text-[hsl(var(--visual-science))]">
                Staff member invited successfully!
              </p>
              <p className="text-xs text-[hsl(var(--visual-science))] mt-1">
                Share this temporary password with the new staff member:
              </p>
              <p className="mt-2 font-mono text-sm bg-white rounded-lg px-3 py-2 border border-[hsl(var(--visual-science)/0.25)] select-all">
                {result.temporaryPassword}
              </p>
            </div>
            <button
              onClick={onCreated}
              className="w-full px-4 py-2 bg-violet-600 text-white rounded-xl text-sm font-semibold hover:bg-violet-700"
            >
              Done
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-1">
              <label htmlFor="staff-name" className="text-xs vi-text-muted font-medium">
                Full Name *
              </label>
              <input
                id="staff-name"
            ...
            <div className="space-y-1">
              <label htmlFor="staff-email" className="text-xs vi-text-muted font-medium">
                Email *
              </label>
              <input
                id="staff-email"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label htmlFor="staff-role" className="text-xs vi-text-muted font-medium">
                  Role *
                </label>
                <select
                  id="staff-role"
                  ...
                  className="w-full px-3 py-2 rounded-xl border vi-border text-sm focus:border-violet-400 outline-none"
                >
              ...
              <div className="space-y-1">
                <label htmlFor="staff-school" className="text-xs vi-text-muted font-medium">
                  Assign to School
                </label>
                <select
                  id="staff-school"
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 px-4 py-2 border vi-border rounded-xl text-sm vi-text-muted hover:vi-bg"
              >
                Cancel
              </button>
              <button
