          ...
          {data.schoolAssignments?.length > 0 ? (
            <div className="space-y-2">
              {data.schoolAssignments.map((a: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between py-2 px-3 rounded-lg vi-bg"
                >
                  <Link
                    href={`/dashboard/district/schools/${a.schoolId}`}
                    className="text-sm font-medium text-[hsl(var(--visual-primary))] hover:underline"
                  >
                    {a.schoolName}
                  </Link>
                  <div className="flex items-center gap-2">
                    {a.roleAtSchool && (
                      <span className="text-xs vi-text-muted">{a.roleAtSchool}</span>
                    )}
