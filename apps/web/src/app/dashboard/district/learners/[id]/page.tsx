        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="vi-card p-6 space-y-3">
            ...
            <InfoRow
              ...
              value={l.functioningLevel?.replace(/_/g, " ") || "—"}
            />
          ...
          <div className="space-y-4">
            {data.school && (
              <div className="vi-card p-6 space-y-3">
                ...
                <Link
                  href={`/dashboard/district/schools/${data.school.id}`}
                  className="text-[hsl(var(--visual-primary))] hover:underline text-sm font-medium"
                >
                  {data.school.name}
                </Link>
              </div>
          ...
          {(data.ieps || []).length === 0 ? (
            ...
            (data.ieps ?? []).map((iep: any) => (
              <div key={iep.id} className="vi-card p-6 space-y-3">
                ...
                {iep.goals?.length > 0 && (
                  <div>
                    <p className="text-xs vi-text-muted font-medium mb-1">
                      Goals ({iep.goals.length})
                    </p>
                    <ul className="space-y-1">
                      {iep.goals.map((g: any, i: number) => (
                        <li key={i} className="text-sm vi-text-muted vi-bg rounded-lg px-3 py-2">
        ...
        <div className="vi-card p-6">
          {data.sensoryProfile ? (
            <div className="space-y-3">
              ...
              <pre className="text-xs vi-text-muted vi-bg rounded-xl p-4 overflow-auto">
                {JSON.stringify(data.sensoryProfile, null, 2)}
              </pre>
            </div>
          ) : (
