        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm vi-text-muted mt-1">
              Monitor your district&apos;s platform usage against your subscription limits.
            </p>
          </div>
        </div>
        {usage && (
      ...
      {usage && (
        <>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <UsageCard
              label="Learners"
              used={usage.learners.used}
              limit={usage.learners.limit}
              Icon={GraduationCap}
            ...
            <UsageCard
              label="Schools"
              used={usage.schools.used}
              limit={usage.schools.limit}
              Icon={School}
          ...
          <div className="vi-card p-6 space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <AiUsageStat
                label="AI Calls"
                value={usage.aiCalls.used}
                limit={usage.aiCalls.limit}
                period={usage.aiCalls.period}
              />
              <AiUsageStat
                label="Tutor Sessions"
                value={usage.tutorSessions.used}
                limit={usage.tutorSessions.limit}
                period={usage.tutorSessions.period}
              />
              <AiUsageStat
                label="Storage"
                value={usage.storage.usedMb}
                limit={usage.storage.limitMb}
                unit="MB"
              />
            </div>
            <p className="text-xs vi-text-muted">
              AI usage tracking will populate as learners interact with tutors. Limits are based on
...
function SeatToolsCard({ usage }: { usage: UsageData | null }) {
  ...
  async function submitSeat(e: React.FormEvent) {
    ...
    if (!Number.isInteger(n) || n < 1) {
      setMsg({ type: "error", text: "Enter a valid seat count" });
      return;
    }
    setBusy("seat");
    setMsg(null);
    try {
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok) {
        setMsg({ type: "error", text: body.error || "Request failed" });
        return;
      }
      setShowSeat(false);
      setRequested("");
  ...
  return (
    <div className="vi-surface-soft rounded-2xl border border-[hsl(var(--visual-primary)/0.3)] p-6 space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-[hsl(var(--visual-primary))] mt-1">
            Request more seats, export the current roster, or download your full district activity
            log.
          </p>
        </div>
      </div>
      {msg && (
        <div
          className={`px-3 py-2 rounded-lg text-sm ${msg.type === "ok" ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-800"}`}
        >
          {msg.text}
        </div>
      )}
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => downloadCsv("/api/district/roster.csv", "roster.csv", "roster")}
        ...
        <button
          onClick={() =>
            downloadCsv("/api/district/activity/export?format=csv", "activity.csv", "export")
          }
      ...
      {showSeat && (
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === "Escape") setShowSeat(false);
          }}
        >
          ...
          >
            ...
            <p className="text-xs vi-text-muted">
              Current allocation: {usage?.users.limit?.toLocaleString() || "—"} user seats. Billing
              will reach out to confirm before changes are applied.
            </p>
            <label className="block">
              <span className="text-xs font-semibold vi-text-muted uppercase tracking-wide">
                New total seats
              </span>
              <input
                type="number"
            ...
            <div className="flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setShowSeat(false)}
                className="px-4 py-2 rounded-xl border vi-border vi-text text-sm"
              >
                Cancel
              </button>
              <button
...
function UsageCard({
  ...
  return (
    <div className="vi-card p-5 hover:shadow-md transition">
      ...
      <p
        className={`text-xs mt-1 font-medium ${isHigh ? "text-[hsl(var(--visual-sel))]" : "vi-text-muted"}`}
      >
        {pct}% used
      </p>
    </div>
...
function AiUsageStat({
  ...
  return (
    <div className="vi-bg rounded-xl p-4 border vi-border">
      ...
      <p className="text-xl font-bold vi-text mt-1">
        {value.toLocaleString()} {unit || ""}
      </p>
      <p className="text-xs vi-text-muted mt-0.5">
        {pct}% of {limit.toLocaleString()} {unit || ""} limit{period ? ` (${period})` : ""}
      </p>
    </div>
  );
}
