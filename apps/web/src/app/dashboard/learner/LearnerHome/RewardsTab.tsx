...
function rarityIcon(rarity: string) {
  if (rarity === "legendary")
    return <Gem className="w-7 h-7 text-[hsl(var(--visual-primary))]" strokeWidth={2.5} />;
  if (rarity === "epic")
    return (
      <Star
        className="w-7 h-7 text-[hsl(var(--visual-math))] fill-[hsl(var(--visual-math))]"
        strokeWidth={2.5}
      />
    );
  if (rarity === "rare")
    return (
      <Star
        className="w-7 h-7 text-[hsl(var(--visual-reading))] fill-[hsl(var(--visual-reading))]"
        strokeWidth={2.5}
      />
    );
...
export function RewardsTab({ badges, streakCurrent, streakLongest, onNavigate }: RewardsTabProps) {
  ...
  const chip = (key: "shop" | "badges" | "leaderboard") => {
    const map = {
      shop: {
        color: "hsl(var(--visual-science))",
        tint: "hsl(var(--visual-science) / 0.12)",
        label: t("shop"),
        path: "/dashboard/learner/shop",
        Icon: ShoppingBag,
      },
      badges: {
        color: "hsl(var(--visual-sel))",
        tint: "hsl(var(--visual-sel) / 0.16)",
        label: t("rewards_badges"),
        path: "/dashboard/learner/badges",
        Icon: Award,
      },
      leaderboard: {
        color: "hsl(var(--visual-primary))",
        tint: "hsl(var(--visual-primary) / 0.12)",
        label: t("leaderboard"),
        path: "/dashboard/learner/leaderboard",
        Icon: Trophy,
      },
    } as const;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-extrabold text-slate-900">
              {t("streak_days_label", { count: streakCurrent })}
            </span>
          </div>
          <span className="text-sm text-slate-500 font-semibold">
            {t("streak_best_label", { count: streakLongest })}
          </span>
        </div>
      </LearnerCard>
    </div>
