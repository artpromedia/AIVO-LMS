                ...
                >
                  <Image
                    ...
                    style={{
                      height: 44,
                      width: "auto",
                      maxWidth: sidebarExpanded ? 180 : 60,
                  ...
                  {learners.map((l) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => {
                          setActiveLearner(l);
                          router.push(`/dashboard/parent/learner/${l.id}`);
                        }}
                      ...
                      >
                        <div
                          className={`w-9 h-9 rounded-2xl flex items-center justify-center text-sm font-black text-white shrink-0 shadow-sm ${active ? "bg-[hsl(var(--visual-primary))]" : "bg-[hsl(var(--visual-text-muted))]"}`}
                        >
                          {l.name.charAt(0)}
                        </div>
                    ...
                    <li key={item.key}>
                      ...
                      >
                        ...
                        {item.key === "inbox" && unreadCount > 0 && (
                          <span
                            aria-hidden="true"
                            className={`${sidebarExpanded ? "ml-auto" : "absolute -top-1 -right-1"} bg-[hsl(var(--visual-math))] text-white text-[10px] font-black rounded-full min-w-[20px] h-5 px-1 flex items-center justify-center shadow-md`}
                          >
                  ...
                  <div className="relative lg:hidden">
                    ...
                    >
                      ...
                      {activeLearner?.gradeLevel && (
                        <span className="text-xs text-[hsl(var(--visual-primary)/0.7)] font-semibold">
                          Gr {activeLearner.gradeLevel}
                        </span>
                      )}
                      ...
                      <div className="absolute top-full mt-2 right-0 vi-card py-2 min-w-52 z-50">
                        {learners.map((l) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-2xl flex items-center justify-center text-xs font-black text-white shrink-0 ${active ? "bg-[hsl(var(--visual-primary))]" : "bg-[hsl(var(--visual-text-muted))]"}`}
                              >
                                {l.name.charAt(0)}
                              </div>
                              <div>
            ...
            <div className="flex items-center justify-around h-16 px-2">
              {BOTTOM_TABS.map((tab) => {
                const active = tab.href
                  ? isActive(tab.href, tab.matchExact)
                  : tab.key === "child"
                    ? pathname.includes("/learner/") && !pathname.includes("/progress")
                    : tab.key === "progress"
                      ? pathname.includes("/progress")
                      : false;
                const Icon = tab.Icon;
                const label = t(`nav_${tab.key}`, {
                  defaultValue: tab.key.charAt(0).toUpperCase() + tab.key.slice(1),
                });
                return (
                  <button
                    key={tab.key}
                  ...
                  >
                    <span
                      className={`flex items-center justify-center w-10 h-7 rounded-full transition ${active ? "bg-[hsl(var(--visual-primary)/0.12)]" : ""}`}
                    >
                      <Icon size={20} strokeWidth={active ? 2.75 : 2.25} aria-hidden="true" />
                    </span>
                    <span
                      className={`text-[10px] leading-none ${active ? "font-black" : "font-bold"}`}
                    >
                      {label}
                    </span>
                    {tab.key === "inbox" && unreadCount > 0 && (
                      <span
                        aria-hidden="true"
                        className="absolute top-1 right-[28%] bg-[hsl(var(--visual-math))] text-white text-[8px] font-black rounded-full w-4 h-4 flex items-center justify-center shadow-md"
                      >
                        {unreadCount > 9 ? "9+" : unreadCount}
                      </span>
                    )}
