...
export default function TransitionPlanningPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/learning/gradebook/${learnerId}`, {
  ...
  const threshold = MILESTONE_THRESHOLDS[level] ?? 0.7;
  ...
  const nextGrade =
    gradeIdx >= 0 && gradeIdx + 1 < GRADE_BANDS.length ? GRADE_BANDS[gradeIdx + 1] : null;
  const nextLevelIdx = FUNCTIONING_LEVELS.indexOf(level) + 1;
  const nextLevel =
    nextLevelIdx > 0 && nextLevelIdx < FUNCTIONING_LEVELS.length
      ? FUNCTIONING_LEVELS[nextLevelIdx]
      : null;
          ...
          <>
            <section className="vi-card p-6 space-y-4">
              <h2 className="font-heading font-bold text-xl vi-text flex items-center gap-2">
                <GraduationCap size={20} className="text-[hsl(var(--visual-primary))]" /> Current
                placement
              </h2>
            ...
            <section className="vi-card p-6 space-y-4">
              ...
              <div className="space-y-3">
                <Milestone
                  ...
                  title={
                    nextGrade ? `Move to ${nextGrade} grade band` : "Highest grade band reached"
                  }
                  detail={
                    nextGrade
                      ? `Requires ${Math.round(threshold * 100)}% average mastery (currently ${Math.round(overallReadiness * 100)}%)`
                      : "Continue advanced enrichment"
                  }
                />
                <Milestone
                  done={false}
                  title={
                    nextLevel
                      ? `Step up to ${nextLevel} functioning level`
                      : "Highest functioning level"
                  }
                  detail={
                    nextLevel
                      ? "Re-assess after sustained mastery improvements over 30 days"
            ...
            <section className="vi-card p-6 space-y-4">
              ...
              {inProgress.length === 0 ? (
                <p className="text-sm vi-text-muted">
                  No active skills below the mastery threshold.
                </p>
              ) : (
                <ul className="space-y-2">
                  {inProgress.slice(0, 10).map((g, i) => (
                    <li
                      key={i}
                      className="flex items-center gap-3 py-2 border-b vi-border last:border-0"
                    >
                      <ArrowRight
                        size={16}
                        className="text-[hsl(var(--visual-primary))] shrink-0"
                      />
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-800 truncate">{g.skill}</p>
                        <p className="text-xs vi-text-muted">
                          {g.subject} · {Math.round(g.masteryScore * 100)}% /{" "}
                          {Math.round(threshold * 100)}% needed
                        </p>
                      </div>
                    </li>
                  ))}
...
function Milestone({ done, title, detail }: { done: boolean; title: string; detail: string }) {
  return (
    <div
      className={`flex items-start gap-3 p-4 rounded-2xl ${done ? "bg-emerald-50" : "bg-slate-50"}`}
    >
      <CheckCircle2
        size={20}
        className={done ? "text-emerald-600 shrink-0" : "text-slate-300 shrink-0"}
      />
      <div>
