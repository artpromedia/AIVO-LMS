...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.mfaPending) {
        router.push(
          `/verify-mfa?token=${encodeURIComponent(data.mfaToken)}&method=${encodeURIComponent(data.mfaMethod || "email")}&returnTo=/dashboard/admin`,
        );
        return;
      }
      await refreshToken();
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-base text-white/50 font-body mt-4 max-w-sm leading-relaxed">
                Secure access to the AIVO management console. Monitor services, manage users, and
