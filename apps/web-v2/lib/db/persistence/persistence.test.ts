...
describe("weeklyCurriculum memory adapter", () => {
  const tenantId = "t_demo";
  const learnerId = "lrn_1";
  function mkUpload(over: Partial<import("@/lib/db/types").CurriculumUpload>) {
    const base = {
      id: `cu_${Math.random().toString(36).slice(2)}`,
      tenantId,
      learnerId,
      uploadedBy: "u_1",
      uploaderRole: "parent" as const,
      subject: "math",
      title: "Week",
  ...
  it("archives a prior undated active row in the same subject on create", async () => {
    const store = getPersistence().weeklyCurriculum;
    const first = await store.create({
      upload: mkUpload({ id: "cu_a", title: "First" }),
      archiveSupersededBefore: null,
    });
    const second = await store.create({
      upload: mkUpload({ id: "cu_b", title: "Second" }),
      archiveSupersededBefore: null,
    });
    const list = await store.listForLearner(learnerId, tenantId);
    const byId = new Map(list.map((u) => [u.id, u]));
    expect(byId.get(first.id)?.status).toBe("archived");
    expect(byId.get(second.id)?.status).toBe("active");
  });

  it("keeps a current plan active when a future-dated plan is added", async () => {
    const store = getPersistence().weeklyCurriculum;
    const todayIso = new Date().toISOString();
    const future = new Date(Date.now() + 14 * 86400000).toISOString();
    const current = await store.create({
      upload: mkUpload({ id: "cu_cur", weekStart: todayIso, weekEnd: todayIso }),
      archiveSupersededBefore: todayIso,
    });
    await store.create({
      upload: mkUpload({ id: "cu_fut", weekStart: future, weekEnd: future }),
      archiveSupersededBefore: future,
    });
    const list = await store.listForLearner(learnerId, tenantId, { status: "active" });
    expect(list.some((u) => u.id === current.id)).toBe(true);
    expect(list.some((u) => u.id === "cu_fut")).toBe(true);
  });

  it("deletes tenant-scoped", async () => {
    const store = getPersistence().weeklyCurriculum;
    await store.create({ upload: mkUpload({ id: "cu_del" }), archiveSupersededBefore: null });
    expect(await store.delete("cu_del", "t_other")).toBe(false);
    expect(await store.delete("cu_del", tenantId)).toBe(true);
    expect(await store.getById("cu_del", tenantId)).toBeNull();
  });
});
