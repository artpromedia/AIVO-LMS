 ...
 *
 * Motion lives in CSS (see preview.css): the idle "breathe" and the greeting
 * wave are layered on top of these static poses and are paused under Calm /
 * `prefers-reduced-motion`, so every expression reads correctly with zero
 * animation.
 *
 * Preview-only: this file lives under app/design-preview and ships no
 * production art. Phase 2 promotes a hardened version into the shared kit.
 */
/* eslint-disable no-restricted-syntax -- This file is original SVG
 * illustration: gradient stops and fills are the artwork's internal palette
 * (mascot plumage, beak, eyes), not themeable UI chrome. The no-hardcoded-hex
 * rule targets app surfaces that must read brand tokens; illustration colors
 * are intentionally fixed, exactly like the existing static art in
 * public/images/mascots. Surrounding UI still consumes the brand tokens. */
import * as React from "react";

export type AivoExpression =
  | "greeting"
  | "encouraging"
  | "celebrating"
  | "thinking"
...
function Flourish({ expression }: { expression: AivoExpression }) {
  ...
  if (expression === "resting") {
    return (
      <g className="aivo-zzz" fill="#8b6cf6" fontFamily="ui-sans-serif, sans-serif" fontWeight="700">
        <text x="146" y="58" fontSize="16">z</text>
        <text x="158" y="44" fontSize="22">Z</text>
      </g>
    );
  }
  return null;
