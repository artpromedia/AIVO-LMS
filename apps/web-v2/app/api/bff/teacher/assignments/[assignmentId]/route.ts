...
export async function PATCH(req: Request, { params }: Params): Promise<NextResponse> {
  ...
  try {
    ...
    if (Array.isArray(body.learnerIds) && body.learnerIds.every((s) => typeof s === "string")) {
    ...
    if (!updated) return fail({ ...ERRORS.NOT_FOUND, message: "Assignment not found." }, requestId);
...
export async function DELETE(req: Request, { params }: Params): Promise<NextResponse> {
  ...
  try {
    ...
    const roleErr = requireRole(session!, ["teacher"], requestId);
    if (roleErr) return roleErr;
    const removed = deleteTeacherAssignment(assignmentId, session!.userId, session!.tenantId);
    if (!removed) return fail({ ...ERRORS.NOT_FOUND, message: "Assignment not found." }, requestId);
