...
import { getUtilizationSeries } from "@/lib/billing/district-pool";
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (session!.role !== "platform_admin" && session!.tenantId !== tenantId) {
      return fail(
        { ...ERRORS.FORBIDDEN_TENANT, message: "Not allowed to view usage for that tenant." },
        requestId,
      );
    }

    const granularity: "day" | "month" = granularityParam === "day" ? "day" : "month";

    const data = getUtilizationSeries(tenantId, granularity);
    return ok({ usage: data }, requestId);
  } catch (e) {
