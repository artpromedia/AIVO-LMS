...
/**
 * web-v2 home page — "A calmer, more personal way to learn" redesign.
 *
 * This is the public landing page for the signed-in app (app.aivolearning.com)
 * and is kept visually in sync with apps/marketing's home page so both public
 * surfaces match. The shared design lives in two places per app:
 *   - the section markup below (sourced from `root.home.*` translations), and
 *   - the HomeHeroDevice / LearnerFunctionShowcase scene components.
 *
 * Differences from apps/marketing:
 *   - i18n namespace is `root.home` (apps/web-v2/lib/i18n/messages), not
 *     `marketing.home`.
 *   - The hero is session-aware: a signed-in visitor sees "Continue as {role}"
 *     instead of the marketing trial CTAs.
 *   - Marketing-only routes (/for-parents, /privacy-policy, /demo, …) do not
 *     exist in web-v2, so those links resolve against the marketing site via
 *     `marketingHref()` (mirrors site-footer.tsx).
 *   - Chrome is web-v2's SiteHeader / SiteFooter.
 */
...
export default async function Home() {
  ...
  const signupHref = "/signup?plan=free";
  const demoHref = marketingHref("/demo");
  ...
  const adultViews = [
    {
      ...
      href: marketingHref("/for-parents"),
    },
    {
      Icon: GraduationCap,
      ...
      href: marketingHref("/for-teachers"),
    },
    {
      Icon: School,
      ...
      href: marketingHref("/for-schools"),
    },
  ];
  ...
  const trustItems = [
    { Icon: UserCheck, n: 1, href: marketingHref("/privacy-policy") },
    { Icon: ShieldCheck, n: 2, href: marketingHref("/coppa-compliance") },
    { Icon: FileCheck2, n: 3, href: marketingHref("/ferpa-compliance") },
    { Icon: Lock, n: 4, href: marketingHref("/security") },
    { Icon: Accessibility, n: 5, href: marketingHref("/accessibility") },
  ].map(({ Icon, n, href }) => ({
    Icon,
    href,
  ...
  return (
    <div className="min-h-screen bg-white text-iw-ink">
      ...
      <main id="main">
          ...
          <div className="mx-auto grid max-w-6xl items-center gap-16 px-6 md:px-8 lg:grid-cols-2">
            <div className="max-w-xl">
              ...
              <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                {session ? (
                  <>
                    <Link href={ROLE_HOME[session.role]} className={heroPrimaryBtn}>
                    ...
                    <Link href="/login" className={heroSecondaryBtn}>
                      {t("switch_role")}
                    </Link>
                  </>
                ) : (
                  <>
                    <Link href={signupHref} className={heroPrimaryBtn}>
                      {t("hero_cta_primary")}
                      <ArrowRight
                        className="h-4 w-4 transition-transform group-hover:translate-x-1"
                        aria-hidden="true"
        ...
        <section className="bg-[var(--aivo-cloud-50)] py-20 md:py-28" aria-labelledby="steps-heading">
          <div className="mx-auto max-w-6xl px-6 md:px-8">
            <h2
              id="steps-heading"
              className="mx-auto max-w-3xl text-center font-iw-display text-3xl font-bold tracking-tight text-iw-ink md:text-4xl"
        ...
        {/* Trust details */}
        <section className="bg-[var(--aivo-cloud-50)] py-20 md:py-28" aria-labelledby="trust-heading">
          <div className="mx-auto max-w-6xl px-6 md:px-8">
            <h2
              id="trust-heading"
              className="mx-auto max-w-3xl text-center font-iw-display text-3xl font-bold tracking-tight text-iw-ink md:text-4xl"
            ...
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
              {trustItems.map(({ Icon, title, desc, cta, href }) => (
                ...
                >
                  ...
                  <a
                    href={href}
                    className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--aivo-sensory-primary)] transition-all hover:gap-2.5"
                  >
                    {cta}
                    <ArrowRight className="h-4 w-4" aria-hidden="true" />
                  </a>
          ...
          <div className="relative mx-auto max-w-6xl overflow-hidden rounded-[2rem] bg-iw-primary px-8 py-16 text-white shadow-soft-5 md:px-16 md:py-20">
            ...
            <div className="relative z-10 flex flex-col items-center gap-10 text-center md:flex-row md:justify-between md:text-left">
              <div className="max-w-xl">
                <h2
                  id="cta-heading"
                  className="font-iw-display text-3xl font-bold leading-tight tracking-tight md:text-5xl"
                >
                  {t("cta_heading")}
                </h2>
