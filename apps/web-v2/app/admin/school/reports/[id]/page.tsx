...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!schema) {
    return shell(
      <>
        <PageHeader eyebrow="School admin · Reports" title="Report" />
        <EmptyState
          title="Report not found"
          description="No report exists with that id in your scope."
          action={
            <Link href="/admin/school/reports" className="text-sm font-semibold text-iw-primary">
              Back to reports
            </Link>
          }
        />
  ...
  return shell(
    <>
      ...
      <Card className="p-[var(--aivo-density-card-pad)]">
        ...
        <ReportParamForm schema={schema} basePath="/admin/school/reports" />
      </Card>
