    ...
    <fieldset className="space-y-4 rounded-md border border-aivo-border p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div>
          ...
          <Input id="map-email" value={value.emailDomain} onChange={(e) => set("emailDomain", e.target.value)} placeholder="district.k12.us" className="mt-1" />
        </div>
        <div>
          <Label htmlFor="map-student">{t("student_role")}</Label>
          <Input id="map-student" value={value.studentRole} onChange={(e) => set("studentRole", e.target.value)} className="mt-1" />
        </div>
        <div>
          <Label htmlFor="map-teacher">{t("teacher_role")}</Label>
          <Input id="map-teacher" value={value.teacherRole} onChange={(e) => set("teacherRole", e.target.value)} className="mt-1" />
        </div>
      </div>
      <div>
        ...
        <div className="mt-2 space-y-2">
          ...
          {gradeEntries.map(([sis, aivo], i) => (
            <div key={i} className="flex items-center gap-2">
              <Input aria-label={t("sis_grade")} value={sis} onChange={(e) => setGradeKey(sis, e.target.value)} className="flex-1" placeholder="06" />
              <span aria-hidden className="text-aivo-ink-soft">→</span>
              <Input aria-label={t("aivo_grade")} value={aivo} onChange={(e) => setGradeVal(sis, e.target.value)} className="flex-1" placeholder="Grade 6" />
              <Button type="button" variant="ghost" size="sm" onClick={() => removeGrade(sis)} aria-label={t("remove")}>✕</Button>
            </div>
          ))}
        </div>
        <Button type="button" variant="outline" size="sm" className="mt-2" onClick={() => { if (value.gradeMap[""] === undefined) set("gradeMap", { ...value.gradeMap, "": "" }); }}>
          {t("add_grade")}
        </Button>
      </div>
    </fieldset>
