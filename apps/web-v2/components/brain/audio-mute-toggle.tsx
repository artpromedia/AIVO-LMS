 ...
 * `localStorage["aivo:audio-muted"]` via `lib/audio` so every chime and
 * swell in the app respects the same single source of truth.
 *
 * Ported from `aivo-ai-learning/apps/web/src/components/AudioMuteToggle.tsx`.
 */
export default function AudioMuteToggle({ className = "", compact = false }: AudioMuteToggleProps) {
  // Hydration-safe: start in a known state and resolve in useEffect so
