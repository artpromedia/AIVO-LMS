...
function NewsletterSignup() {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6" aria-label="Subscribe to AIVO newsletter">
      ...
      <div className="flex items-center w-full max-w-sm rounded-full bg-white border border-slate-200 focus-within:border-[var(--aivo-color-primary,#7c3aed)] focus-within:ring-2 focus-within:ring-purple-300/30 transition overflow-hidden">
        <input
          type="email"
          required
          autoComplete="email"
        ...
        <button
          type="submit"
          disabled={status === "submitting" || status === "success"}
          className="m-1 inline-flex items-center justify-center gap-1.5 rounded-full bg-[var(--aivo-color-primary,#7c3aed)] px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition disabled:opacity-70 shrink-0 min-h-10"
        >
          {status === "submitting" && (
            <>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-8">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 mb-12">
          <div className="col-span-2">
            ...
            <div className="inline-flex items-center gap-2 bg-purple-50 border border-purple-100 px-4 py-2 rounded-full text-[var(--aivo-color-primary,#7c3aed)]">
          ...
          {FOOTER_SECTIONS.map((section) => (
            <div key={section.titleKey}>
              <h4 className="text-sm font-heading font-bold text-slate-900 mb-4">
                {t(section.titleKey)}
              </h4>
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.labelKey}>
                    <Link
                      href={link.href}
                      className="text-sm text-slate-600 hover:text-[var(--aivo-color-primary,#7c3aed)] transition"
                    >
                      {t(link.labelKey)}
                    </Link>
                  </li>
        ...
        <div className="border-t border-slate-200/70 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="inline-flex items-center gap-2 bg-purple-50 px-4 py-2 rounded-full text-[var(--aivo-color-primary,#7c3aed)]">
            <ShieldCheck className="w-4 h-4" aria-hidden="true" />
