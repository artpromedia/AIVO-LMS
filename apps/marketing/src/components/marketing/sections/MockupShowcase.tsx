    ...
    <section className="mb-14" aria-labelledby={headingId}>
      <div className="grid gap-8 lg:grid-cols-2 items-center">
        <div className={reverse ? "lg:order-2" : ""}>
          ...
          <h2
            id={headingId}
            className="mt-2 font-heading text-2xl md:text-3xl font-bold text-slate-900"
          >
            {heading}
          </h2>
          <div className="mt-3 text-slate-600 font-body leading-relaxed">{body}</div>
          {bullets && bullets.length > 0 && (
            <ul className="mt-5 space-y-2">
              {bullets.map((b) => (
                <li key={b} className="flex gap-2 text-slate-700 font-body text-sm">
