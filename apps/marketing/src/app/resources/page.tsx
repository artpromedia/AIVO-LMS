...
export default async function ResourcesPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-b from-iw-purple-50/40 via-white to-white py-16 md:py-20">
      ...
      <main className="mx-auto max-w-5xl space-y-16 px-6 py-12 md:px-8 md:py-16">
        <section aria-labelledby="guides-heading">
          <h2 id="guides-heading" className="font-heading text-2xl font-bold text-iw-ink">
            {t("guides_heading")}
          </h2>
          <p className="mt-1 font-body text-iw-ink-muted">{t("guides_subheading")}</p>
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            {guides.map((g) => (
              <Link
                key={g.slug}
                href={`/guides/${g.slug}`}
                className="group block rounded-iw-card-lg border border-iw-border bg-white p-6 transition hover:border-iw-purple-200 hover:shadow-soft-3 md:p-8"
              >
                <span
                ...
                <h3 className="mt-4 font-heading text-xl font-bold text-iw-ink group-hover:text-iw-primary">
                  {g.title}
                </h3>
                <p className="mt-2 font-body text-iw-ink-muted">{g.excerpt}</p>
                <p className="mt-3 font-body text-sm text-iw-ink-muted">{g.readTime}</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section aria-labelledby="blog-heading">
          <h2 id="blog-heading" className="font-heading text-2xl font-bold text-iw-ink">
            {t("blog_heading")}
          </h2>
          <p className="mt-1 font-body text-iw-ink-muted">{t("blog_subheading")}</p>
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            {blog.map((p) => (
              <Link
                key={p.slug}
              ...
              >
                ...
                <p className="mt-2 font-body text-iw-ink-muted">{p.excerpt}</p>
                <p className="mt-3 font-body text-sm text-iw-ink-muted">{p.readTime}</p>
              </Link>
            ))}
          </div>
        </section>
